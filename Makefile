...
TO_DOCKERIZE ?= apigw apiserver npm cmd tpm netagent spyglass evtsmgr tsm evtsproxy vos citadel rollout vtsa orchhub vcsim pegasus perseus alertmgr turret
...
UI_BUILD_CONTAINER ?= pens-ui-bld:v0.30
DIND_CONTAINER ?= pens-dind:v0.5
E2E_CONTAINER ?= pens-e2e:v0.7
...
helper-containers:
	...
	@cd tools/docker-files/elasticsearch; docker build -t ${REGISTRY_URL}/elasticsearch-cluster:v0.17 .
