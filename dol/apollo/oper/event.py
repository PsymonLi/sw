...
class EventObject(base.ConfigObjectBase):
    ...
    def Show(self):
        logger.info(f"Event Object: {self}")
        return

    def GetGrpcEventMessage(self):
    ...
    def RequestEventStream(self):
        def GetOperInfoRequestIterator():
            messages = [self.GetGrpcEventMessage()]
            for m in messages:
                yield m
        ...
        logger.info(f"received events stream {self.Stream} from {self.Node}")
    ...
    def GetEvents(self):
        ...
        for infoResp in self.Stream:
            assert (infoResp.Status == types_pb2.API_STATUS_OK),\
                   f"Event get failed on {self.Node}, rc {infoResp.Status}"
            assert (infoResp.InfoType == oper_pb2.OPER_INFO_TYPE_EVENT),\
                   f"Unexpected oper info {infoResp.InfoType} received on "\
                   f"{self.Node} instead of events"
            yield infoResp.EventInfo

    def DrainEvents(self, spec=None):
        try:
            events = self.GetEvents()
            while True:
                logger.info(f"Draining old events, {next(events)}")
        except:
            pass
        return True

class EventObjectsClient(base.ConfigClientBase):
    def __init__(self):
        super().__init__(api.PenOperObjectTypes.ALERTS, Resmgr.MAX_OPER)
        # one event object per node
        self.Objs = dict()
    ...
    def CreateObjects(self, node):
        ...
        # start event streams request
        obj = self.Objects(node)
        if not obj:
            return False
        obj.RequestEventStream()
        return True

client = EventObjectsClient()
