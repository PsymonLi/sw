...
class Prof1():
    ...
    def create_udp_template(self, ip_gen, delay, loop, cps):
        ...
        # template
        temp_c1 = ASTFTCPClientTemplate(program=prog_c,
                                        ip_gen=ip_gen,
                                        cps=cps,
                                        port=port)
        temp_s1 = ASTFTCPServerTemplate(program=prog_s,
    ...
    def create_profile(self, client_server_pair, max_active_flow, cps):
        ports = 1
        self.crunch_number(max_active_flow,
                           len(client_server_pair) + ports, cps)
        templates = []
        ip_count = 15
        for _ in range(ports):
            for client_ip, server_ip in client_server_pair:
                # ip generator
                ip_gen_s1 = ASTFIPGenDist(ip_range=[client_ip, str(ipaddress.IPv4Address(client_ip) + ip_count)],
                                          distribution="seq")
                ip_gen_c1 = ASTFIPGenDist(ip_range=[server_ip, str(ipaddress.IPv4Address(server_ip) + ip_count)],
                                          distribution="seq")
                ...
                # templates
                templates.append(self.create_tcp_template(ip_gen1, self.delay, self.loop,
                                 self.cps))
                templates.append(self.create_udp_template(ip_gen1, self.delay, self.loop,
                                 self.cps))
