...
def __execute_athena_client(node, tc, cmd_options):
    req = api.Trigger_CreateExecuteCommandsRequest(serial = True)
    cmd = __CMDPATH + __CMDBASE + __CMDSEP + cmd_options
    api.Trigger_AddNaplesCommand(req, node, cmd, timeout = __CMD_TIMEOUT)
    tc.resp = api.Trigger(req)

def ScriptExecCommand(node, tc, script_dir, script_name, log_file = None):
    ...
    __execute_athena_client(node, tc, cmd_options)

    if log_file is not None:
        req = api.Trigger_CreateExecuteCommandsRequest(serial = True)
        api.Trigger_AddNaplesCommand(req, node, "cat %s" % log_file)
