...
func (w *Watcher) handleApisrvWatch(ctx context.Context, apicl apiclient.Services) {
	...
	// smartNIC Watcher
	smartNICWatcher, err := apicl.ClusterV1().DistributedServiceCard().Watch(ctx, &api.ListWatchOptions{FieldChangeSelector: []string{"ObjectMeta.Labels", "Spec", "Status.AdmissionPhase", "Status.Conditions"}})
	if err != nil {
		log.Errorf("Failed to start node watcher (%s)\n", err)
		return
	}
