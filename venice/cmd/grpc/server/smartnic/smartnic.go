...
func (s *RPCServer) UpdateSmartNIC(updObj *cluster.DistributedServiceCard) (*cluster.DistributedServiceCard, error) {
	...
	updateAPIServer := !runtime.FilterUpdate(refObj.Status, updObj.Status, []string{"LastTransitionTime"}, []string{"Conditions", "AdmissionPhase", "IPConfig.IPAddress", "BGPStatus"})
	...
	if updObj.Status.AdmissionPhase == cluster.DistributedServiceCardStatus_DECOMMISSIONED.String() {
		...
		// Copy the entire state because it may contain useful info
		refObj.Status = updObj.Status
	} else {
