...
// PolicerProfile is a wrapper object that implements additional functionality
...
func (obj *PolicerProfile) SetInternal() {
	obj.internal = true
}

func (obj *PolicerProfile) Write() error {
	// if there is no API server to connect to, we are done
	if (obj.ctrler == nil) || (obj.ctrler.resolver == nil) || obj.ctrler.apisrvURL == "" {
		return nil
	}
	...
	obj.ctrler.stats.Counter("PolicerProfile_Writes").Inc()

	// write to api server
	if obj.ObjectMeta.ResourceVersion != "" {
		// update it
		for i := 0; i < maxApisrvWriteRetry; i++ {
			_, err = apicl.NetworkV1().PolicerProfile().UpdateStatus(context.Background(), &obj.PolicerProfile)
			if err == nil {
				break
			}
			time.Sleep(time.Millisecond * 100)
		...
		//  create
		_, err = apicl.NetworkV1().PolicerProfile().Create(context.Background(), &obj.PolicerProfile)
	}

	return nil
}

// PolicerProfileHandler is the event handler for PolicerProfile object
type PolicerProfileHandler interface {
	OnPolicerProfileCreate(obj *PolicerProfile) error
	OnPolicerProfileUpdate(oldObj *PolicerProfile, newObj *network.PolicerProfile) error
	OnPolicerProfileDelete(obj *PolicerProfile) error
	GetPolicerProfileWatchOptions() *api.ListWatchOptions
	OnPolicerProfileReconnect()
}

// OnPolicerProfileCreate is a dummy handler used in init if no one registers the handler
func (ctrler CtrlDefReactor) OnPolicerProfileCreate(obj *PolicerProfile) error {
	log.Info("OnPolicerProfileCreate is not implemented")
	return nil
}

// OnPolicerProfileUpdate is a dummy handler used in init if no one registers the handler
func (ctrler CtrlDefReactor) OnPolicerProfileUpdate(oldObj *PolicerProfile, newObj *network.PolicerProfile) error {
	log.Info("OnPolicerProfileUpdate is not implemented")
	return nil
}

// OnPolicerProfileDelete is a dummy handler used in init if no one registers the handler
func (ctrler CtrlDefReactor) OnPolicerProfileDelete(obj *PolicerProfile) error {
	log.Info("OnPolicerProfileDelete is not implemented")
	return nil
}

// GetPolicerProfileWatchOptions is a dummy handler used in init if no one registers the handler
func (ctrler CtrlDefReactor) GetPolicerProfileWatchOptions() *api.ListWatchOptions {
	log.Info("GetPolicerProfileWatchOptions is not implemented")
	opts := &api.ListWatchOptions{}
	return opts
}

// OnPolicerProfileReconnect is a dummy handler used in init if no one registers the handler
func (ctrler CtrlDefReactor) OnPolicerProfileReconnect() {
	log.Info("OnPolicerProfileReconnect is not implemented")
	return
}

// handlePolicerProfileEvent handles PolicerProfile events from watcher
func (ct *ctrlerCtx) handlePolicerProfileEvent(evt *kvstore.WatchEvent) error {

	if ct.objResolver == nil {
		return ct.handlePolicerProfileEventNoResolver(evt)
	}

	switch tp := evt.Object.(type) {
	case *network.PolicerProfile:
		eobj := evt.Object.(*network.PolicerProfile)
		kind := "PolicerProfile"

		log.Infof("Watcher: Got %s watch event(%s): {%+v}", kind, evt.Type, eobj)

		ctx := &policerprofileCtx{event: evt.Type,
			obj: &PolicerProfile{PolicerProfile: *eobj, ctrler: ct}}
		...
		return err
	default:
		ct.logger.Fatalf("API watcher Found object of invalid type: %v on PolicerProfile watch channel", tp)
	}

	return nil
}

// handlePolicerProfileEventNoResolver handles PolicerProfile events from watcher
func (ct *ctrlerCtx) handlePolicerProfileEventNoResolver(evt *kvstore.WatchEvent) error {
	switch tp := evt.Object.(type) {
	case *network.PolicerProfile:
		eobj := evt.Object.(*network.PolicerProfile)
		kind := "PolicerProfile"
		...
		policerprofileHandler := handler.(PolicerProfileHandler)
		// handle based on event type
		ctrlCtx := &policerprofileCtx{event: evt.Type, obj: &PolicerProfile{PolicerProfile: *eobj, ctrler: ct}}
		switch evt.Type {
		case kvstore.Created:
			fallthrough
		case kvstore.Updated:
			...
			if err != nil {
				ct.addObject(ctrlCtx)
				ct.stats.Counter("PolicerProfile_Created_Events").Inc()

				// call the event handler
				ctrlCtx.Lock()
				err = policerprofileHandler.OnPolicerProfileCreate(ctrlCtx.obj)
				ctrlCtx.Unlock()
				if err != nil {
					ct.logger.Errorf("Error creating %s %+v. Err: %v", kind, ctrlCtx.obj.GetObjectMeta(), err)
					ct.delObject(kind, ctrlCtx.GetKey())
				...
				ctrlCtx := fobj.(*policerprofileCtx)
				ct.stats.Counter("PolicerProfile_Updated_Events").Inc()
				ctrlCtx.Lock()
				p := network.PolicerProfile{Spec: eobj.Spec,
					ObjectMeta: eobj.ObjectMeta,
					TypeMeta:   eobj.TypeMeta,
					Status:     eobj.Status}

				err = policerprofileHandler.OnPolicerProfileUpdate(ctrlCtx.obj, &p)
				ctrlCtx.obj.PolicerProfile = *eobj
				ctrlCtx.Unlock()
				if err != nil {
					ct.logger.Errorf("Error creating %s %+v. Err: %v", kind, ctrlCtx.obj.GetObjectMeta(), err)
					return err
		...
		case kvstore.Deleted:
			...
			obj := fobj.(*PolicerProfile)
			ct.stats.Counter("PolicerProfile_Deleted_Events").Inc()
			obj.Lock()
			err = policerprofileHandler.OnPolicerProfileDelete(obj)
			obj.Unlock()
			if err != nil {
				ct.logger.Errorf("Error deleting %s: %+v. Err: %v", kind, obj.GetObjectMeta(), err)
			}
...
type policerprofileCtx struct {
	ctkitBaseCtx
	event kvstore.WatchEventType
	obj   *PolicerProfile //
	//   newObj     *network.PolicerProfile //update
	newObj *policerprofileCtx //update
}

func (ctx *policerprofileCtx) References() map[string]apiintf.ReferenceObj {
	if ctx.references == nil {
		resp := make(map[string]apiintf.ReferenceObj)
		ctx.references = resp
		ctx.obj.References(ctx.obj.GetObjectMeta().Name, ctx.obj.GetObjectMeta().Namespace, resp)
...
func (ctx *policerprofileCtx) GetKey() string {
	return ctx.obj.MakeKey("network")

}

func (ctx *policerprofileCtx) IsInternal() bool {
	return ctx.obj.internal
}

func (ctx *policerprofileCtx) GetKind() string {
	return ctx.obj.GetKind()
}

func (ctx *policerprofileCtx) GetResourceVersion() string {
	return ctx.obj.GetResourceVersion()
}

func (ctx *policerprofileCtx) SetEvent(event kvstore.WatchEventType) {
	ctx.event = event
}

func (ctx *policerprofileCtx) SetNewObj(newObj apiintf.CtkitObject) {
	if newObj == nil {
		ctx.newObj = nil
	} else {
		ctx.newObj = newObj.(*policerprofileCtx)
		ctx.newObj.obj.HandlerCtx = ctx.obj.HandlerCtx
		ctx.references = newObj.References()
	}
}

func (ctx *policerprofileCtx) GetNewObj() apiintf.CtkitObject {
	return ctx.newObj
}

func (ctx *policerprofileCtx) Copy(obj apiintf.CtkitObject) {
	ctx.obj.PolicerProfile = obj.(*policerprofileCtx).obj.PolicerProfile
	ctx.SetWatchTs(obj.GetWatchTs())
}

func (ctx *policerprofileCtx) Lock() {
	ctx.obj.Lock()
}

func (ctx *policerprofileCtx) Unlock() {
	ctx.obj.Unlock()
}

func (ctx *policerprofileCtx) GetObjectMeta() *api.ObjectMeta {
	return ctx.obj.GetObjectMeta()
}

func (ctx *policerprofileCtx) RuntimeObject() runtime.Object {
	var v interface{}
	v = ctx.obj
	return v.(runtime.Object)
}

func (ctx *policerprofileCtx) WorkFunc(context context.Context) error {
	var err error
	evt := ctx.event
	ct := ctx.obj.ctrler
	kind := "PolicerProfile"
	...
	case kvstore.Created:
		ctx.obj.Lock()
		err = policerprofileHandler.OnPolicerProfileCreate(ctx.obj)
		ctx.obj.Unlock()
		if err != nil {
			ct.logger.Errorf("Error creating %s %+v. Err: %v", kind, ctx.obj.GetObjectMeta(), err)
			ctx.SetEvent(kvstore.Deleted)
	...
	case kvstore.Updated:
		ct.stats.Counter("PolicerProfile_Updated_Events").Inc()
		ctx.obj.Lock()
		p := network.PolicerProfile{Spec: ctx.newObj.obj.Spec,
			ObjectMeta: ctx.newObj.obj.ObjectMeta,
			TypeMeta:   ctx.newObj.obj.TypeMeta,
			Status:     ctx.newObj.obj.Status}
		err = policerprofileHandler.OnPolicerProfileUpdate(ctx.obj, &p)
		ctx.obj.Unlock()
		if err != nil {
			ct.logger.Errorf("Error updating %s %+v. Err: %v", kind, ctx.obj.GetObjectMeta(), err)
		}
	case kvstore.Deleted:
		ctx.obj.Lock()
		err = policerprofileHandler.OnPolicerProfileDelete(ctx.obj)
		ctx.obj.Unlock()
		if err != nil {
			ct.logger.Errorf("Error deleting %s %+v. Err: %v", kind, ctx.obj.GetObjectMeta(), err)
		}
...
// handlePolicerProfileEventParallel handles PolicerProfile events from watcher
func (ct *ctrlerCtx) handlePolicerProfileEventParallel(evt *kvstore.WatchEvent) error {

	if ct.objResolver == nil {
		return ct.handlePolicerProfileEventParallelWithNoResolver(evt)
	}
	...
	case *network.PolicerProfile:
		...
		ctx := &policerprofileCtx{event: evt.Type, obj: &PolicerProfile{PolicerProfile: *eobj, ctrler: ct}}
		ctx.SetWatchTs(evt.WatchTS)
...
// handlePolicerProfileEventParallel handles PolicerProfile events from watcher
func (ct *ctrlerCtx) handlePolicerProfileEventParallelWithNoResolver(evt *kvstore.WatchEvent) error {
	switch tp := evt.Object.(type) {
	case *network.PolicerProfile:
		eobj := evt.Object.(*network.PolicerProfile)
		kind := "PolicerProfile"
		...
		case kvstore.Updated:
			workFunc := func(ctx context.Context, ctrlCtx shardworkers.WorkObj) error {
				var err error
				workCtx := ctrlCtx.(*policerprofileCtx)
				eobj := workCtx.obj
				fobj, err := ct.getObject(kind, workCtx.GetKey())
				if err != nil {
					ct.addObject(workCtx)
					ct.stats.Counter("PolicerProfile_Created_Events").Inc()
					eobj.Lock()
					err = policerprofileHandler.OnPolicerProfileCreate(eobj)
					eobj.Unlock()
					if err != nil {
						ct.logger.Errorf("Error creating %s %+v. Err: %v", kind, eobj.GetObjectMeta(), err)
						ct.delObject(kind, workCtx.GetKey())
					...
					workCtx = fobj.(*policerprofileCtx)
					obj := workCtx.obj
					ct.stats.Counter("PolicerProfile_Updated_Events").Inc()
					obj.Lock()
					p := network.PolicerProfile{Spec: eobj.Spec,
					...
					err = policerprofileHandler.OnPolicerProfileUpdate(obj, &p)
					if err != nil {
						ct.logger.Errorf("Error creating %s %+v. Err: %v", kind, obj.GetObjectMeta(), err)
					} else {
						workCtx.obj.PolicerProfile = p
					}
					obj.Unlock()
				}
				workCtx.SetWatchTs(evt.WatchTS)
			...
			ctrlCtx := &policerprofileCtx{event: evt.Type, obj: &PolicerProfile{PolicerProfile: *eobj, ctrler: ct}}
			ct.runFunction("PolicerProfile", ctrlCtx, workFunc)
		case kvstore.Deleted:
			workFunc := func(ctx context.Context, ctrlCtx shardworkers.WorkObj) error {
				var err error
				workCtx := ctrlCtx.(*policerprofileCtx)
...
// diffPolicerProfile does a diff of PolicerProfile objects between local cache and API server
func (ct *ctrlerCtx) diffPolicerProfile(apicl apiclient.Services) {
	opts := api.ListWatchOptions{}

	// get a list of all objects from API server
	objlist, err := apicl.NetworkV1().PolicerProfile().List(context.Background(), &opts)
	if err != nil {
		ct.logger.Errorf("Error getting a list of objects. Err: %v", err)
		return
	}

	ct.logger.Infof("diffPolicerProfile(): PolicerProfileList returned %d objects", len(objlist))

	// build an object map
	objmap := make(map[string]*network.PolicerProfile)
	for _, obj := range objlist {
		objmap[obj.GetKey()] = obj
	}

	list, err := ct.PolicerProfile().List(context.Background(), &opts)
	if err != nil && !strings.Contains(err.Error(), "not found in local cache") {
		ct.logger.Infof("Failed to get a list of objects. Err: %s", err)
		return
	}
	...
	for _, obj := range list {
		_, ok := objmap[obj.GetKey()]
		if !ok {
			ct.logger.Infof("diffPolicerProfile(): Deleting existing object %#v since its not in apiserver", obj.GetKey())
			evt := kvstore.WatchEvent{
				Type:   kvstore.Deleted,
				Key:    obj.GetKey(),
				Object: &obj.PolicerProfile,
			}
			ct.handlePolicerProfileEvent(&evt)
		}
	}

	// trigger create event for all others
	for _, obj := range objlist {
		ct.logger.Infof("diffPolicerProfile(): Adding object %#v", obj.GetKey())
		evt := kvstore.WatchEvent{
			Type:   kvstore.Created,
			Key:    obj.GetKey(),
			Object: obj,
...
func (ct *ctrlerCtx) runPolicerProfileWatcher() {
	kind := "PolicerProfile"
	...
	opts := policerprofileHandler.GetPolicerProfileWatchOptions()
	...
	cancelEntry := ct.watchCancel[kind]
	ct.Unlock()
	logger := ct.logger.WithContext("submodule", "PolicerProfileWatcher")
	for {
		if ctx.Err() != nil {
			return
		}
	...
	go func() {
		defer cancelEntry.wg.Done()
		defer ct.waitGrp.Done()
		ct.stats.Counter("PolicerProfile_Watch").Inc()
		defer ct.stats.Counter("PolicerProfile_Watch").Dec()
		...
		for {
			...
			if err != nil {
				logger.Warnf("Failed to connect to gRPC server [%s]\n", ct.apisrvURL)
				ct.stats.Counter("PolicerProfile_ApiClientErr").Inc()
			} else {
				logger.Infof("API client connected {%+v}", apicl)

				// PolicerProfile object watcher
				wt, werr := apicl.NetworkV1().PolicerProfile().Watch(ctx, opts)
				if werr != nil {
					select {
					case <-ctx.Done():
						logger.Infof("watch %s cancelled", kind)
				...
				ct.diffPolicerProfile(apicl)
				policerprofileHandler.OnPolicerProfileReconnect()
			...
			innerLoop:
				for {
					...
					case evt, ok := <-wt.EventChan():
						if !ok {
							logger.Error("Error receiving from apisrv watcher")
							ct.stats.Counter("PolicerProfile_WatchErrors").Inc()
							break innerLoop
						}

						// handle event in parallel
						ct.handlePolicerProfileEventParallel(evt)
					}
				}
				apicl.Close()
			}
...
// WatchPolicerProfile starts watch on PolicerProfile object
func (ct *ctrlerCtx) WatchPolicerProfile(handler PolicerProfileHandler) error {
	kind := "PolicerProfile"
	...
	ct.Unlock()
	if ok {
		return fmt.Errorf("PolicerProfile watcher already exists")
	}
	...
	// run PolicerProfile watcher in a go routine
	ct.runPolicerProfileWatcher()

	return nil
}

// StopWatchPolicerProfile stops watch on PolicerProfile object
func (ct *ctrlerCtx) StopWatchPolicerProfile(handler PolicerProfileHandler) error {
	kind := "PolicerProfile"
	...
	ct.Unlock()
	if !ok {
		return fmt.Errorf("PolicerProfile watcher does not exist")
	}
...
// PolicerProfileAPI returns
type PolicerProfileAPI interface {
	Create(obj *network.PolicerProfile) error
	SyncCreate(obj *network.PolicerProfile) error
	Update(obj *network.PolicerProfile) error
	SyncUpdate(obj *network.PolicerProfile) error
	Label(obj *api.Label) error
	Delete(obj *network.PolicerProfile) error
	SyncDelete(obj *network.PolicerProfile) error
	Find(meta *api.ObjectMeta) (*PolicerProfile, error)
	List(ctx context.Context, opts *api.ListWatchOptions) ([]*PolicerProfile, error)
	ApisrvList(ctx context.Context, opts *api.ListWatchOptions) ([]*network.PolicerProfile, error)
	Watch(handler PolicerProfileHandler) error
	ClearCache(handler PolicerProfileHandler)
	StopWatch(handler PolicerProfileHandler) error
}

// dummy struct that implements PolicerProfileAPI
type policerprofileAPI struct {
	ct *ctrlerCtx
}

// Create creates PolicerProfile object
func (api *policerprofileAPI) Create(obj *network.PolicerProfile) error {
	if api.ct.resolver != nil {
		apicl, err := api.ct.apiClient()
		if err != nil {
			api.ct.logger.Errorf("Error creating API server clent. Err: %v", err)
		...
		_, err = apicl.NetworkV1().PolicerProfile().Create(context.Background(), obj)
		if err != nil && strings.Contains(err.Error(), "AlreadyExists") {
			_, err = apicl.NetworkV1().PolicerProfile().Update(context.Background(), obj)

		}
		return err
	}

	api.ct.handlePolicerProfileEvent(&kvstore.WatchEvent{Object: obj, Type: kvstore.Created})
	return nil
}

// SyncCreate creates PolicerProfile object and updates the cache
func (api *policerprofileAPI) SyncCreate(obj *network.PolicerProfile) error {
	newObj := obj
	evtType := kvstore.Created
	var writeErr error
	if api.ct.resolver != nil {
		...
		newObj, writeErr = apicl.NetworkV1().PolicerProfile().Create(context.Background(), obj)
		if writeErr != nil && strings.Contains(writeErr.Error(), "AlreadyExists") {
			newObj, writeErr = apicl.NetworkV1().PolicerProfile().Update(context.Background(), obj)
			evtType = kvstore.Updated
		}
	}

	if writeErr == nil {
		api.ct.handlePolicerProfileEvent(&kvstore.WatchEvent{Object: newObj, Type: evtType})
	}
	return writeErr
}

// Update triggers update on PolicerProfile object
func (api *policerprofileAPI) Update(obj *network.PolicerProfile) error {
	if api.ct.resolver != nil {
		apicl, err := api.ct.apiClient()
		if err != nil {
			api.ct.logger.Errorf("Error creating API server clent. Err: %v", err)
	...
	api.ct.handlePolicerProfileEvent(&kvstore.WatchEvent{Object: obj, Type: kvstore.Updated})
	return nil
}

// SyncUpdate triggers update on PolicerProfile object and updates the cache
func (api *policerprofileAPI) SyncUpdate(obj *network.PolicerProfile) error {
	if api.ct.objResolver != nil {
		log.Fatal("Cannot use Sync update when object resolver is enabled on ctkit")
	}
	newObj := obj
	...
	if writeErr == nil {
		api.ct.handlePolicerProfileEvent(&kvstore.WatchEvent{Object: newObj, Type: kvstore.Updated})
	}

	return writeErr
}

// Label labels PolicerProfile object
func (api *policerprofileAPI) Label(obj *api.Label) error {
	if api.ct.resolver != nil {
		apicl, err := api.ct.apiClient()
		if err != nil {
			api.ct.logger.Errorf("Error creating API server clent. Err: %v", err)
		...
		_, err = apicl.NetworkV1().PolicerProfile().Label(context.Background(), obj)
		return err
	}
	...
	writeObj := ctkitObj.PolicerProfile
	writeObj.Labels = obj.Labels

	api.ct.handlePolicerProfileEvent(&kvstore.WatchEvent{Object: &writeObj, Type: kvstore.Updated})
	return nil
}

// Delete deletes PolicerProfile object
func (api *policerprofileAPI) Delete(obj *network.PolicerProfile) error {
	if api.ct.resolver != nil {
		apicl, err := api.ct.apiClient()
		if err != nil {
			api.ct.logger.Errorf("Error creating API server clent. Err: %v", err)
		...
		_, err = apicl.NetworkV1().PolicerProfile().Delete(context.Background(), &obj.ObjectMeta)
		return err
	}

	api.ct.handlePolicerProfileEvent(&kvstore.WatchEvent{Object: obj, Type: kvstore.Deleted})
	return nil
}

// SyncDelete deletes PolicerProfile object and updates the cache
func (api *policerprofileAPI) SyncDelete(obj *network.PolicerProfile) error {
	var writeErr error
	if api.ct.resolver != nil {
		apicl, err := api.ct.apiClient()
		if err != nil {
		...
		_, writeErr = apicl.NetworkV1().PolicerProfile().Delete(context.Background(), &obj.ObjectMeta)
	}
...
// MakeKey generates a KV store key for the object
func (api *policerprofileAPI) getFullKey(tenant, name string) string {
	if tenant != "" {
		return fmt.Sprint(globals.ConfigRootPrefix, "/", "network", "/", "policer-profile", "/", tenant, "/", name)
	}
	return fmt.Sprint(globals.ConfigRootPrefix, "/", "network", "/", "policer-profile", "/", name)
}

// Find returns an object by meta
func (api *policerprofileAPI) Find(meta *api.ObjectMeta) (*PolicerProfile, error) {
	// find the object
	obj, err := api.ct.FindObject("PolicerProfile", meta)
	if err != nil {
		return nil, err
	}
...
// List returns a list of all PolicerProfile objects
func (api *policerprofileAPI) List(ctx context.Context, opts *api.ListWatchOptions) ([]*PolicerProfile, error) {
	var objlist []*PolicerProfile
	objs, err := api.ct.List("PolicerProfile", ctx, opts)
	...
	for _, obj := range objs {
		switch tp := obj.(type) {
		case *PolicerProfile:
			eobj := obj.(*PolicerProfile)
			objlist = append(objlist, eobj)
		default:
			log.Fatalf("Got invalid object type %v while looking for PolicerProfile", tp)
		}
	}
...
// ApisrvList returns a list of all PolicerProfile objects from apiserver
func (api *policerprofileAPI) ApisrvList(ctx context.Context, opts *api.ListWatchOptions) ([]*network.PolicerProfile, error) {
	if api.ct.resolver != nil {
		apicl, err := api.ct.apiClient()
		if err != nil {
			api.ct.logger.Errorf("Error creating API server clent. Err: %v", err)
		...
		return apicl.NetworkV1().PolicerProfile().List(context.Background(), opts)
	}
	...
	var ret []*network.PolicerProfile
	for _, obj := range ctkitObjs {
		ret = append(ret, &obj.PolicerProfile)
	}
	return ret, nil
}

// Watch sets up a event handlers for PolicerProfile object
func (api *policerprofileAPI) Watch(handler PolicerProfileHandler) error {
	api.ct.startWorkerPool("PolicerProfile")
	return api.ct.WatchPolicerProfile(handler)
}

// StopWatch stop watch for Tenant PolicerProfile object
func (api *policerprofileAPI) StopWatch(handler PolicerProfileHandler) error {
	api.ct.Lock()
	worker := api.ct.workPools["PolicerProfile"]
	api.ct.Unlock()
	// Don't call stop with ctkit lock. Lock might be taken when an event comes in for the worker
	if worker != nil {
		worker.Stop()
	}
	return api.ct.StopWatchPolicerProfile(handler)
}

// ClearCache removes all PolicerProfile objects in ctkit
func (api *policerprofileAPI) ClearCache(handler PolicerProfileHandler) {
	api.ct.delKind("PolicerProfile")
}

// PolicerProfile returns PolicerProfileAPI
func (ct *ctrlerCtx) PolicerProfile() PolicerProfileAPI {
	ct.Lock()
	defer ct.Unlock()
	kind := "PolicerProfile"
	if _, ok := ct.apiInfMap[kind]; !ok {
		s := &policerprofileAPI{ct: ct}
		ct.apiInfMap[kind] = s
	}
	return ct.apiInfMap[kind].(*policerprofileAPI)
}
