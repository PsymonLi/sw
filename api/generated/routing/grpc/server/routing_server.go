...
func (s *sroutingRoutingBackend) regMsgsFunc(l log.Logger, scheme *runtime.Scheme) {
	...
	s.Messages = map[string]apiserver.Message{
		...
		"routing.EmptyReq": apisrvpkg.NewMessage("routing.EmptyReq"),
		"routing.Health": apisrvpkg.NewMessage("routing.Health").WithKeyGenerator(func(i interface{}, prefix string) string {
			if i == nil {
				r := routing.Health{}
			...
			var n *routing.Health
			if v, ok := i.(routing.Health); ok {
				n = &v
			} else if v, ok := i.(*routing.Health); ok {
				n = v
			} else {
				return nil
			}
			...
			return func(oldObj runtime.Object) (runtime.Object, error) {
				...
				if ret, ok := oldObj.(*routing.Health); ok {
		...
		"routing.HealthSpec":   apisrvpkg.NewMessage("routing.HealthSpec"),
		"routing.HealthStatus": apisrvpkg.NewMessage("routing.HealthStatus"),
		...
		"routing.NeighborFilter": apisrvpkg.NewMessage("routing.NeighborFilter").WithKeyGenerator(func(i interface{}, prefix string) string {
			if i == nil {
				r := routing.NeighborFilter{}
				return r.MakeKey(prefix)
			}
			r := i.(routing.NeighborFilter)
			return r.MakeKey(prefix)
		}).WithObjectVersionWriter(func(i interface{}, version string) interface{} {
			r := i.(routing.NeighborFilter)
			r.Kind = "NeighborFilter"
			r.APIVersion = version
			return r
		}).WithKvUpdater(func(ctx context.Context, kvs kvstore.Interface, i interface{}, prefix string, create bool, updateFn kvstore.UpdateFunc) (interface{}, error) {
			r := i.(routing.NeighborFilter)
			...
			if create {
				if updateFn != nil {
					upd := &routing.NeighborFilter{}
					n, err := updateFn(upd)
					if err != nil {
						l.ErrorLog("msg", "could not create new object", "err", err)
						return nil, err
					}
					new := n.(*routing.NeighborFilter)
					new.TypeMeta = r.TypeMeta
					new.GenerationID = "1"
					new.UUID = r.UUID
					new.CreationTime = r.CreationTime
				...
				var cur routing.NeighborFilter
				err = kvs.Get(ctx, key, &cur)
				if err != nil {
					l.ErrorLog("msg", "trying to update an object that does not exist", "key", key, "err", err)
					return nil, err
