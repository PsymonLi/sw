...
type snetworkPolicerBackend struct {
...
func (s *snetworkPolicerBackend) regMsgsFunc(l log.Logger, scheme *runtime.Scheme) {
	l.Infof("registering message for snetworkPolicerBackend")
	s.Messages = map[string]apiserver.Message{

		"network.PolicerAction":   apisrvpkg.NewMessage("network.PolicerAction"),
		"network.PolicerCriteria": apisrvpkg.NewMessage("network.PolicerCriteria"),
		"network.PolicerProfile": apisrvpkg.NewMessage("network.PolicerProfile").WithKeyGenerator(func(i interface{}, prefix string) string {
			if i == nil {
				r := network.PolicerProfile{}
				return r.MakeKey(prefix)
			}
			r := i.(network.PolicerProfile)
			return r.MakeKey(prefix)
		}).WithObjectVersionWriter(func(i interface{}, version string) interface{} {
			r := i.(network.PolicerProfile)
			r.Kind = "PolicerProfile"
			...
			if create {
				if updateFn != nil {
					upd := &network.PolicerProfile{}
					n, err := updateFn(upd)
					if err != nil {
						l.ErrorLog("msg", "could not create new object", "err", err)
						return nil, err
					}
					new := n.(*network.PolicerProfile)
				...
				if updateFn != nil {
					...
					var cur network.PolicerProfile
			...
			var n *network.PolicerProfile
			if v, ok := i.(network.PolicerProfile); ok {
				n = &v
			} else if v, ok := i.(*network.PolicerProfile); ok {
				n = v
			} else {
				return nil
			}
			return func(oldObj runtime.Object) (runtime.Object, error) {
				...
				o := oldObj.(*network.PolicerProfile)
		...
		"network.PolicerProfileSpec":   apisrvpkg.NewMessage("network.PolicerProfileSpec"),
		"network.PolicerProfileStatus": apisrvpkg.NewMessage("network.PolicerProfileStatus"),
...
func (s *snetworkPolicerBackend) regSvcsFunc(ctx context.Context, logger log.Logger, grpcserver *rpckit.RPCServer, scheme *runtime.Scheme) {

}

func (s *snetworkPolicerBackend) regWatchersFunc(ctx context.Context, logger log.Logger, grpcserver *rpckit.RPCServer, scheme *runtime.Scheme) {

}

func (s *snetworkPolicerBackend) CompleteRegistration(ctx context.Context, logger log.Logger,
...
func (s *snetworkPolicerBackend) Reset() {
	cleanupRegistration()
}
