...
type snetworkVr_peering_groupBackend struct {
...
func (s *snetworkVr_peering_groupBackend) regMsgsFunc(l log.Logger, scheme *runtime.Scheme) {
	l.Infof("registering message for snetworkVr_peering_groupBackend")
	s.Messages = map[string]apiserver.Message{

		"network.VirtualRouterPeeringGroup": apisrvpkg.NewMessage("network.VirtualRouterPeeringGroup").WithKeyGenerator(func(i interface{}, prefix string) string {
			if i == nil {
				r := network.VirtualRouterPeeringGroup{}
				return r.MakeKey(prefix)
			}
			r := i.(network.VirtualRouterPeeringGroup)
			return r.MakeKey(prefix)
		}).WithObjectVersionWriter(func(i interface{}, version string) interface{} {
			r := i.(network.VirtualRouterPeeringGroup)
			r.Kind = "VirtualRouterPeeringGroup"
			...
			if create {
				if updateFn != nil {
					upd := &network.VirtualRouterPeeringGroup{}
					n, err := updateFn(upd)
					if err != nil {
						l.ErrorLog("msg", "could not create new object", "err", err)
						return nil, err
					}
					new := n.(*network.VirtualRouterPeeringGroup)
				...
				if updateFn != nil {
					into := &network.VirtualRouterPeeringGroup{}
					...
					var cur network.VirtualRouterPeeringGroup
			...
			if create {
				...
				if updatefn != nil {
					...
					r = *robj.(*network.VirtualRouterPeeringGroup)
			...
			var n *network.VirtualRouterPeeringGroup
			if v, ok := i.(network.VirtualRouterPeeringGroup); ok {
				n = &v
			} else if v, ok := i.(*network.VirtualRouterPeeringGroup); ok {
				n = v
			} else {
				return nil
			}
			return func(oldObj runtime.Object) (runtime.Object, error) {
				...
				o := oldObj.(*network.VirtualRouterPeeringGroup)
			...
			return func(oldObj runtime.Object) (runtime.Object, error) {
				if oldObj == nil {
					rete := &network.VirtualRouterPeeringGroup{}
					rete.TypeMeta, rete.ObjectMeta, rete.Spec = n.TypeMeta, n.ObjectMeta, n.Spec
					rete.GenerationID = "1"
					return rete, nil
				}
				if ret, ok := oldObj.(*network.VirtualRouterPeeringGroup); ok {
		...
		"network.VirtualRouterPeeringGroupSpec":   apisrvpkg.NewMessage("network.VirtualRouterPeeringGroupSpec"),
		"network.VirtualRouterPeeringGroupStatus": apisrvpkg.NewMessage("network.VirtualRouterPeeringGroupStatus"),
		"network.VirtualRouterPeeringRoute":       apisrvpkg.NewMessage("network.VirtualRouterPeeringRoute"),
		"network.VirtualRouterPeeringRouteTable":  apisrvpkg.NewMessage("network.VirtualRouterPeeringRouteTable"),
		"network.VirtualRouterPeeringSpec":        apisrvpkg.NewMessage("network.VirtualRouterPeeringSpec"),
...
func (s *snetworkVr_peering_groupBackend) regSvcsFunc(ctx context.Context, logger log.Logger, grpcserver *rpckit.RPCServer, scheme *runtime.Scheme) {

}

func (s *snetworkVr_peering_groupBackend) regWatchersFunc(ctx context.Context, logger log.Logger, grpcserver *rpckit.RPCServer, scheme *runtime.Scheme) {

}

func (s *snetworkVr_peering_groupBackend) CompleteRegistration(ctx context.Context, logger log.Logger,
...
func (s *snetworkVr_peering_groupBackend) Reset() {
	cleanupRegistration()
}
