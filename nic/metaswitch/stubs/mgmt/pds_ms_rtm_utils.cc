...
pds_ms_fill_amb_cipr_rtm (AMB_GEN_IPS *mib_msg, pds_ms_config_t *conf)
{
    ...
    if (conf->row_status != AMB_ROW_DESTROY)
    {
        ...
        // Increase RPM Dead wait timer
        // This helps during hitless upgrade by having RTM on domain A retain
        // the BGP routes even after BGP disable.
        // Assumption is that by this time either the switchover to Domain B
        ...
        data->dead_rpm_tmr = 10;  // in minutes
