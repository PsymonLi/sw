...
static constexpr int k_l3_if_id  = 1;
static constexpr int k_l3_if_id_2  = 2;
static constexpr int k_lo_if_id  = 3;
static constexpr int k_tunnel_id  = 500;
static int g_mirror_id  = 601;

// UUID as generated by DOL
static inline pds_obj_key_t
dol_intf_uuid_from_objid (uint32_t id, const mac_addr_t& mac)
{
    auto key = test::uuid_from_objid(id, mac);
    key.id[4] = 0x10;
    return key;
}
...
static void create_intf_proto_grpc (bool lo=false, bool second=false, bool update=false,
    ...
    if (lo) {
        ...
        if (second) {
            pds_if.key = dol_intf_uuid_from_objid(k_l3_if_id_2, g_system_mac_addr);
            ...
            pds_if.key = dol_intf_uuid_from_objid(k_l3_if_id, g_system_mac_addr);
            if (input_ip) {
...
static void delete_intf_proto_grpc () {
    ...
    auto key = dol_intf_uuid_from_objid(k_l3_if_id_2, g_system_mac_addr);
    request.add_id(key.id, PDS_MAX_KEY_LEN);
...
static void delete_lo_proto_grpc () {
    ...
    request.add_id(dol_intf_uuid_from_objid(k_lo_if_id, g_system_mac_addr).id,
                   PDS_MAX_KEY_LEN);
...
static void htls_upg_start_test () {
    ...
    printf ("Simulating Hitless upgrade start event...\n");
...
static void htls_upg_rollbk_test () {
    ...
    printf ("Simulating Hitless upgrade test rollback event...\n");
    ret_status = g_cp_test_stub_->HitlessUpgTestEvent(&context, request, &response);
    if (!ret_status.ok() || (response.apistatus() != types::API_STATUS_OK)) {
        printf("%s failed! ret_status=%d (%s) response.status=%d\n",
                __FUNCTION__, ret_status.error_code(), ret_status.error_message().c_str(),
...
static void htls_upg_repeal_test () {
    pds_ms::HitlessUpgTestEventSpec      request;
    pds_ms::HitlessUpgTestResponse     response;
    ClientContext   context;
    Status          ret_status;
    ...
    auto proto_spec = &request;
    proto_spec->set_repeal (true);

    printf ("Simulating Hitless upgrade test repeal event...\n");
    ret_status = g_cp_test_stub_->HitlessUpgTestEvent(&context, request, &response);
    if (!ret_status.ok() || (response.apistatus() != types::API_STATUS_OK)) {
        printf("%s failed! ret_status=%d (%s) response.status=%d\n",
                __FUNCTION__, ret_status.error_code(), ret_status.error_message().c_str(),
...
static void htls_upg_sync_test () {
    ...
    printf ("Simulating Hitless upgrade sync event...\n");
    ret_status = g_cp_test_stub_->HitlessUpgTestEvent(&context, request, &response);
    if (!ret_status.ok() || (response.apistatus() != types::API_STATUS_OK)) {
        printf("%s failed! ret_status=%d (%s) response.status=%d\n",
                __FUNCTION__, ret_status.error_code(), ret_status.error_message().c_str(),
...
static void create_route_proto_grpc (bool second=false) {
    ...
    if (g_node_id == 3) {
        proto_spec->
            set_interfaceid (dol_intf_uuid_from_objid(k_lo_if_id,
...
int main(int argc, char** argv)
{
    ...
    if (argc == 1 || underlay_only) {
        ...
        if (!strcmp(argv[1], "peer_status")) {
            ...
            return 0;
        } else if (!strcmp(argv[1], "htupg-repeal")) {
            htls_upg_repeal_test();
            return 0;
            ...
            return 0;
        } else if (!strcmp(argv[1], "upd-if")) {
            create_intf_proto_grpc(false, true, true);
            return 0;
        }
        return 0;
    }
