...
stats_timer_cb (void *timer, uint32_t timer_id, void *ctxt)
{
    ...
    // system PB stats
    if ( (hal::g_hal_cfg.device_cfg.forwarding_mode == hal::utils::FORWARDING_MODE_CLASSIC) ||
         (hal::g_hal_cfg.device_cfg.forwarding_mode == hal::utils::FORWARDING_MODE_HOSTPIN) ) {
        bzero(&pd_func_args, sizeof(pd::pd_func_args_t));
