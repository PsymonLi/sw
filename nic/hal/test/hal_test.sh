...
CONFIG_PATH=${WS_TOP}/hack/saratk/nic/conf
...
if [[ $valgrind -eq 1 ]]; then
    ...
    echo "Testing LIF: "
    ASIC_MOCK_MODE=1 LD_LIBRARY_PATH=$OBJ WS_TOP=${WS_TOP}/ CONFIG_PATH=${WS_TOP}/hack/saratk/nic/conf $OBJ/lif_test --gtest_output="xml:$GEN_TEST_RESULTS_DIR/lif_test.xml"
    echo "Testing UplinkIf: "
    ASIC_MOCK_MODE=1 LD_LIBRARY_PATH=$OBJ WS_TOP=${WS_TOP}/ CONFIG_PATH=${WS_TOP}/hack/saratk/nic/conf $OBJ/uplinkif_test --gtest_output="xml:$GEN_TEST_RESULTS_DIR/uplinkif_test.xml"
fi
