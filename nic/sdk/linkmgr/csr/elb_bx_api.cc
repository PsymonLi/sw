...
void elb_bx_set_ch_enable(int chip_id, int inst_id, int value) {
    BX_DEBUG_MSG("BX%d ch%d elb_bx_set_ch_enable = %d", inst_id, 0, value);
...
void elb_bx_set_glbl_mode(int chip_id, int inst_id, int value) {
    BX_DEBUG_MSG("BX%d ch%d elb_bx_set_glbl_mode = %d", inst_id, 0, value);
    int rdata = elb_bx_apb_read(chip_id, inst_id, 0x1f10);
...
void elb_bx_set_mtu(int chip_id , int inst_id, int max_value, int jabber_value) {
    BX_DEBUG_MSG("BX%d ch%d elb_bx_set_mtu max_value = %d, jabber_value=%d", inst_id, 0, max_value, jabber_value);
...
void elb_bx_init_start(int chip_id, int inst_id) {
    BX_DEBUG_MSG("BX%d ch%d inside init start", inst_id, 0);

    elb_bx_csr_t & bx_csr = PEN_BLK_REG_MODEL_ACCESS(elb_bx_csr_t, chip_id, inst_id);

    elb_bx_set_ch_enable(chip_id, inst_id, 0x0);
    BX_DEBUG_MSG("BX%d ch%d Set BX fixer timeout to 1024", inst_id, 0);
    bx_csr.cfg_fixer.timeout(1024);
    bx_csr.cfg_fixer.write();
}

void elb_bx_init_done(int chip_id, int inst_id) {
    BX_DEBUG_MSG("BX%d ch%d inside init done", inst_id, 0);
}
...
void elb_bx_mac_stat(int chip_id, int inst_id, int port, int short_report,
    ...
    for (int i = 0; i < 32; i += 1) {
        ...
        //BX_DEBUG_MSG("BX%d ch%d %50s : %" PRIu64 "", inst_id, 0, counter_name.c_str(), (uint64_t) rdata);
...
void elb_bx_set_an_ability(int chip_id, int inst_id, int value) {
    BX_DEBUG_MSG("BX%d ch%d BX: set an ability", inst_id, 0);
    elb_bx_apb_write(chip_id, inst_id, 0x4c01, value);
}

int elb_bx_rd_rx_an_ability(int chip_id, int inst_id) {
    BX_DEBUG_MSG("BX%d ch%d BX: read rx an_ability", inst_id, 0);
    int addr = 0x4c02;
...
void elb_bx_set_an_link_timer(int chip_id, int inst_id, int value) {
    BX_DEBUG_MSG("BX%d ch%d BX: set an_ability = %d", inst_id, 0, value);
    int addr = 0x4c03;
...
void elb_bx_set_tx_rx_enable(int chip_id, int inst_id, int tx_enable, int rx_enable) {
    ...
    BX_DEBUG_MSG("BX%d ch%d Set tx_enable = %d, rx_enable = %d", inst_id, 0, tx_enable, rx_enable);
    int addr = 0x2100;
...
void elb_bx_bist_run(int chip_id, int inst_id, int enable) {
    ...
    BX_DEBUG_MSG("BX%d ch%d elb_bx_bist_run, enable = %d", inst_id, 0, enable);
...
void elb_bx_bist_chk(int chip_id, int inst_id) {
    ...
    BX_DEBUG_MSG("BX%d ch%d elb_bx_bist_chk", inst_id, 0);
    ...
    if (bx_csr.sta_ff_txfifo.bist_done_pass() != 0x1) {
        BX_DEBUG_ERR("BX%d ch%d bx_csr.sta_ff_txfifo.bist_done_pass()", inst_id, 0);
        err = true;
    }
    if (bx_csr.sta_ff_txfifo.bist_done_fail() != 0x0) {
        BX_DEBUG_ERR("BX%d ch%d bx_csr.sta_ff_txfifo.bist_done_fail()", inst_id, 0);
        err = true;
    }
    if (bx_csr.sta_ff_rxfifo.bist_done_pass() != 0x1) {
        BX_DEBUG_ERR("BX%d ch%d bx_csr.sta_ff_rxfifo.bist_done_pass()", inst_id, 0);
        err = true;
    }
    if (bx_csr.sta_ff_rxfifo.bist_done_fail() != 0x0) {
        BX_DEBUG_ERR("BX%d ch%d bx_csr.sta_ff_rxfifo.bist_done_fail()", inst_id, 0);
        err = true;
    }
    if (bx_csr.sta_stats_mem.bist_done_pass() != 0x1) {
        BX_DEBUG_ERR("BX%d ch%d bx_csr.sta_stats_mem.bist_done_pass()", inst_id, 0);
        err = true;
    }
    if (bx_csr.sta_stats_mem.bist_done_fail() != 0x0) {
        BX_DEBUG_ERR("BX%d ch%d bx_csr.sta_stats_mem.bist_done_fail()", inst_id, 0);
        err = true;
    }
    if (!err) {
        BX_DEBUG_MSG("BX%d ch%d PASSED: elb_bx_bist_chk", inst_id, 0);
    }
}
