...
{
    MX_DEBUG_INFO("elb_mx_set_field pos = %d, size = %d, value = 0x%" PRIx64 "", pos, size, value);
    if (data == NULL) {
        MX_DEBUG_ERR("elb_mx_set_field argument data is NULL! Other arguments pos = %d, size = %d, value = 0x%" PRIx64 "", pos, size, value);
        return -1;
    }
    uint64_t mask;
    if (size == 64)
...
elb_mx_get_field (uint64_t data, int pos, int size)
{
    ...
    MX_DEBUG_INFO("elb_mx_get_field pos = %d, size = %d, value = 0x%" PRIx64 "", pos, size, value);
    return value;
}
...
uint64_t elb_mx_apb_read(int chip_id, int inst_id, int addr) {
    ...
    MX_DEBUG_INFO("MX%d elb_mx_apb_read addr = %x, rdata = 0x%" PRIx64 "", inst_id, addr, rdata);
    return rdata;
}
...
void elb_mx_apb_write(int chip_id, int inst_id, int addr, uint64_t data) {
    ...
    MX_DEBUG_INFO("MX%d elb_mx_apb_write addr = %x, data = 0x%" PRIx64 "", inst_id, addr, data);
}
...
elb_mx_set_soft_reset(int chip_id, int inst_id, int port, int ch, int value) {
    MX_DEBUG_MSG("MX%d ch%d elb_mx_set_soft_reset value=%d", inst_id, ch, value);
...
{
    MX_DEBUG_MSG("MX%d elb_mx_set_clock_info value=%d", inst_id, value);
...
elb_mx_set_ch_mode(int chip_id, int inst_id, int ch, int speed, int num_lanes, int fec) {
    ...
    MX_DEBUG_MSG("MX%d ch%d elb_mx_set_ch_mode speed=%d, num_lanes=%d, fec=%d, value=%d", inst_id, ch, speed, num_lanes, fec, value);
    if (value == -1) return -1;
    int addr = elb_mx_mac_t::ELB_MX_MAC_CHMODE_ADDR0 + ch * elb_mx_mac_t::ELB_MX_MAC_CHMODE_ADDR_INC;
    uint64_t data = elb_mx_apb_read(chip_id, inst_id, addr);
...
elb_mx_gmii_lpbk_get (int chip_id, int inst_id, int ch)
{
    ...
    MX_DEBUG_MSG("MX%d ch%d elb_mx_gmii_lpbk_get data=%d", inst_id, ch, (int) data);
    return (int) data;
}
...
{
    MX_DEBUG_MSG("MX%d ch%d elb_mx_gmii_lpbk_set value=%d", inst_id, ch, value);
    int addr = elb_mx_mac_t::ELB_MX_MAC_CHMODE_ADDR0 + ch * elb_mx_mac_t::ELB_MX_MAC_CHMODE_ADDR_INC;
    uint64_t data = elb_mx_apb_read(chip_id, inst_id, addr);
...
elb_mx_pcs_lpbk_get (int chip_id, int inst_id, int ch)
{
    ...
    MX_DEBUG_MSG("MX%d ch%d elb_mx_pcs_lpbk_get data=%d", inst_id, ch, (int) data);
    return (int) data;
}
...
{
    MX_DEBUG_MSG("MX%d ch%d elb_mx_pcs_lpbk_set value=%d", inst_id, ch, value);
...
elb_mx_set_auto_flush(int chip_id, int inst_id, int port, int flush_timeout) {
    MX_DEBUG_MSG("MX%d port%d elb_mx_set_auto_flush timeout=%d", inst_id, port, flush_timeout);
    elb_mx_csr_t & mx_csr = PEN_BLK_REG_MODEL_ACCESS(elb_mx_csr_t, chip_id, inst_id);
...
elb_mx_set_flush(int chip_id, int inst_id, int port, int flush) {
    MX_DEBUG_MSG("MX%d port%d elb_mx_set_flush flush=%d", inst_id, port, flush);
    elb_mx_csr_t & mx_csr = PEN_BLK_REG_MODEL_ACCESS(elb_mx_csr_t, chip_id, inst_id);
    mx_csr.cfg_port[port].read();
...
elb_mx_set_rxsm_enable(int chip_id, int inst_id, int port, int enable) {
    MX_DEBUG_MSG("MX%d port%d elb_mx_set_rxsm_enable enable=%d", inst_id, port, enable);
    elb_mx_csr_t & mx_csr = PEN_BLK_REG_MODEL_ACCESS(elb_mx_csr_t, chip_id, inst_id);
...
elb_mx_set_tx_autodrain(int chip_id, int inst_id, int ch, int value) {
    MX_DEBUG_MSG("MX%d ch%d elb_mx_set_tx_autodrain value=%d", inst_id, ch, value);
...
elb_mx_set_fixer_timeout(int chip_id, int inst_id, int port, int value) {
    MX_DEBUG_MSG("MX%d port%d elb_mx_set_fixer_timeout timeout=%d", inst_id, port, value);
    elb_mx_csr_t & mx_csr = PEN_BLK_REG_MODEL_ACCESS(elb_mx_csr_t, chip_id, inst_id);
...
elb_mx_set_tx_drain(int chip_id, int inst_id, int ch, bool drain) {
    MX_DEBUG_MSG("MX%d ch%d elb_mx_set_tx_drain drain=%d", inst_id, ch, drain);
    int addr = elb_mx_mac_t::ELB_MX_MAC_CHMODE_ADDR0 + ch * elb_mx_mac_t::ELB_MX_MAC_CHMODE_ADDR_INC;
    int value = (drain) ? 1 : 0;
    uint64_t data = elb_mx_apb_read(chip_id, inst_id, addr);
...
elb_mx_slot_gen(int inst_id, int *port_vec, int *port_ch_map, int *port_speed, int *slot_port, int *slot_ch)
{
    ...
    for (int port = 0; port < 5; port++) {
        if (port_vec[port]) {
            ...
            for (int i = 0; i < num_slot; i++) {
                ...
                if (slot_port[j] != 7) {
                    MX_DEBUG_ERR("elb_mx_slot_gen: MX%d port = %d, ch = %d, speed = %d slot_port conflict", inst_id, port, port_ch_map[port], port_speed[port]);
                    return -1;
                } else {
                    slot_port[j] = port;
    ...
    for (int slot = 0; slot < 8 ; slot++) {
        MX_DEBUG_MSG("elb_mx_slot_gen: MX%d slot = %d: slot_port = %d, slot_ch = %d", inst_id, slot, slot_port[slot], slot_ch[slot]);
    }
...
elb_mx_get_slot(int chip_id, int inst_id)
{
    ...
    if ( mx_csr.cfg_mac_slot_select.val().convert_to<int>() == 0  ) {
        ...
        MX_DEBUG_MSG("MX%d elb_mx_get_slot_cfg: slot0_port = %d, slot0_channel = %d", inst_id, mx_csr.cfg_mac_slotA.slot0_port().convert_to<int>(), mx_csr.cfg_mac_slotA.slot0_channel().convert_to<int>());
        MX_DEBUG_MSG("MX%d elb_mx_get_slot_cfg: slot1_port = %d, slot1_channel = %d", inst_id, mx_csr.cfg_mac_slotA.slot1_port().convert_to<int>(), mx_csr.cfg_mac_slotA.slot1_channel().convert_to<int>());
        MX_DEBUG_MSG("MX%d elb_mx_get_slot_cfg: slot2_port = %d, slot2_channel = %d", inst_id, mx_csr.cfg_mac_slotA.slot2_port().convert_to<int>(), mx_csr.cfg_mac_slotA.slot2_channel().convert_to<int>());
        MX_DEBUG_MSG("MX%d elb_mx_get_slot_cfg: slot3_port = %d, slot3_channel = %d", inst_id, mx_csr.cfg_mac_slotA.slot3_port().convert_to<int>(), mx_csr.cfg_mac_slotA.slot3_channel().convert_to<int>());
        MX_DEBUG_MSG("MX%d elb_mx_get_slot_cfg: slot4_port = %d, slot4_channel = %d", inst_id, mx_csr.cfg_mac_slotA.slot4_port().convert_to<int>(), mx_csr.cfg_mac_slotA.slot4_channel().convert_to<int>());
        MX_DEBUG_MSG("MX%d elb_mx_get_slot_cfg: slot5_port = %d, slot5_channel = %d", inst_id, mx_csr.cfg_mac_slotA.slot5_port().convert_to<int>(), mx_csr.cfg_mac_slotA.slot5_channel().convert_to<int>());
        MX_DEBUG_MSG("MX%d elb_mx_get_slot_cfg: slot6_port = %d, slot6_channel = %d", inst_id, mx_csr.cfg_mac_slotA.slot6_port().convert_to<int>(), mx_csr.cfg_mac_slotA.slot6_channel().convert_to<int>());
        MX_DEBUG_MSG("MX%d elb_mx_get_slot_cfg: slot7_port = %d, slot7_channel = %d", inst_id, mx_csr.cfg_mac_slotA.slot7_port().convert_to<int>(), mx_csr.cfg_mac_slotA.slot7_channel().convert_to<int>());
    } else {
        mx_csr.cfg_mac_slotB.read();
        MX_DEBUG_MSG("MX%d elb_mx_get_slot_cfg: slot0_port = %d, slot0_channel = %d", inst_id, mx_csr.cfg_mac_slotB.slot0_port().convert_to<int>(), mx_csr.cfg_mac_slotB.slot0_channel().convert_to<int>());
        MX_DEBUG_MSG("MX%d elb_mx_get_slot_cfg: slot1_port = %d, slot1_channel = %d", inst_id, mx_csr.cfg_mac_slotB.slot1_port().convert_to<int>(), mx_csr.cfg_mac_slotB.slot1_channel().convert_to<int>());
        MX_DEBUG_MSG("MX%d elb_mx_get_slot_cfg: slot2_port = %d, slot2_channel = %d", inst_id, mx_csr.cfg_mac_slotB.slot2_port().convert_to<int>(), mx_csr.cfg_mac_slotB.slot2_channel().convert_to<int>());
        MX_DEBUG_MSG("MX%d elb_mx_get_slot_cfg: slot3_port = %d, slot3_channel = %d", inst_id, mx_csr.cfg_mac_slotB.slot3_port().convert_to<int>(), mx_csr.cfg_mac_slotB.slot3_channel().convert_to<int>());
        MX_DEBUG_MSG("MX%d elb_mx_get_slot_cfg: slot4_port = %d, slot4_channel = %d", inst_id, mx_csr.cfg_mac_slotB.slot4_port().convert_to<int>(), mx_csr.cfg_mac_slotB.slot4_channel().convert_to<int>());
        MX_DEBUG_MSG("MX%d elb_mx_get_slot_cfg: slot5_port = %d, slot5_channel = %d", inst_id, mx_csr.cfg_mac_slotB.slot5_port().convert_to<int>(), mx_csr.cfg_mac_slotB.slot5_channel().convert_to<int>());
        MX_DEBUG_MSG("MX%d elb_mx_get_slot_cfg: slot6_port = %d, slot6_channel = %d", inst_id, mx_csr.cfg_mac_slotB.slot6_port().convert_to<int>(), mx_csr.cfg_mac_slotB.slot6_channel().convert_to<int>());
        MX_DEBUG_MSG("MX%d elb_mx_get_slot_cfg: slot7_port = %d, slot7_channel = %d", inst_id, mx_csr.cfg_mac_slotB.slot7_port().convert_to<int>(), mx_csr.cfg_mac_slotB.slot7_channel().convert_to<int>());
    }
}
...
elb_mx_set_mtu_jabber(int chip_id , int inst_id, int ch, int max_value, int jabber_value) {
    MX_DEBUG_MSG("MX%d ch%d elb_mx_set_mtu_jabber max_value=%d, jabber_value=%d", inst_id, ch, max_value, jabber_value);
    int addr = elb_mx_mac_t::ELB_MX_MAC_CHMODE_ADDR0 + ch * elb_mx_mac_t::ELB_MX_MAC_CHMODE_ADDR_INC;
    uint64_t data = elb_mx_apb_read(chip_id, inst_id, addr);
...
{
    MX_DEBUG_MSG("MX%d ch%d port%d elb_mx_set_pause pri_vec=0x%x, legacy=%d tx_pause_enable=%d rx_pause_enable=%d", inst_id, ch, port, pri_vec, legacy, tx_pause_enable, rx_pause_enable);
    elb_mx_csr_t & mx_csr = PEN_BLK_REG_MODEL_ACCESS(elb_mx_csr_t, chip_id, inst_id);
...
{
    MX_DEBUG_MSG("MX%d ch%d elb_mx_set_pause_src_addr mac_addr = 0x%02x%02x%02x%02x%02x%02x", inst_id, ch, mac_addr[0], mac_addr[1], mac_addr[2], mac_addr[3], mac_addr[4], mac_addr[5]);
    if (mac_addr == NULL) {
...
{
    MX_DEBUG_MSG("MX%d ch%d elb_mx_set_pause_dest_addr mac_addr = 0x%02x%02x%02x%02x%02x%02x", inst_id, ch, mac_addr[0], mac_addr[1], mac_addr[2], mac_addr[3], mac_addr[4], mac_addr[5]);
    if (mac_addr == NULL) {
...
{
    MX_DEBUG_MSG("MX%d ch%d elb_mx_set_pauseontime value = 0x%x", inst_id, ch, value);
    int addr = elb_mx_mac_t::ELB_MX_MAC_CHCONFIG4_ADDR0 + ch * elb_mx_mac_t::ELB_MX_MAC_CHCONFIG4_ADDR_INC;
    uint64_t data = elb_mx_apb_read(chip_id, inst_id, addr);
...
{
    MX_DEBUG_MSG("MX%d ch%d elb_mx_set_pauserefresh value = 0x%x", inst_id, ch, value);
    int addr = elb_mx_mac_t::ELB_MX_MAC_CHCONFIG5_ADDR0 + ch * elb_mx_mac_t::ELB_MX_MAC_CHCONFIG5_ADDR_INC;
    uint64_t data = elb_mx_apb_read(chip_id, inst_id, addr);
...
elb_mx_set_txthresh(int chip_id, int inst_id, int ch, int txwrthresh, int txrdthresh) {
    MX_DEBUG_MSG("MX%d ch%d elb_mx_set_txthresh txwrthresh=0x%x, txrdthresh=0x%x", inst_id, ch, txwrthresh, txrdthresh);
    int addr = elb_mx_mac_t::ELB_MX_MAC_MACCFG_ADDR0 + ch * elb_mx_mac_t::ELB_MX_MAC_MACCFG_ADDR_INC;
    uint64_t data = elb_mx_apb_read(chip_id, inst_id, addr);
...
elb_mx_set_ifglen(int chip_id, int inst_id, int ch, int value) {
    ...
    MX_DEBUG_MSG("MX%d ch%d elb_mx_set_ifglen value = %d", inst_id, ch, value);
    int addr = elb_mx_mac_t::ELB_MX_MAC_CHMODE_ADDR0 + ch * elb_mx_mac_t::ELB_MX_MAC_CHMODE_ADDR_INC;
    uint64_t data = elb_mx_apb_read(chip_id, inst_id, addr);
...
elb_mx_disable_eth_len_err(int chip_id, int inst_id, int ch, int value) {
    MX_DEBUG_MSG("MX%d ch%d elb_mx_disable_eth_len_err value = 0x%x", inst_id, ch, value);
    int addr = elb_mx_mac_t::ELB_MX_MAC_CHCONFIG3_ADDR0 + ch * elb_mx_mac_t::ELB_MX_MAC_CHCONFIG3_ADDR_INC;
    uint64_t data = elb_mx_apb_read(chip_id, inst_id, addr);
...
elb_mx_set_ch_enable(int chip_id, int inst_id, int ch, int txen, int rxen) {
    MX_DEBUG_MSG("MX%d ch%d elb_mx_set_ch_enable txen=%d, rxen=%d", inst_id, ch, txen, rxen);
    int addr = elb_mx_mac_t::ELB_MX_MAC_CHMODE_ADDR0 + ch * elb_mx_mac_t::ELB_MX_MAC_CHMODE_ADDR_INC;
    uint64_t data = elb_mx_apb_read(chip_id, inst_id, addr);
...
elb_mx_set_vlan_check (int chip_id, int inst_id, int ch,
                       ...
                       uint32_t tag3)
{
    MX_DEBUG_MSG("MX%d ch%d elb_mx_set_vlan_check num_tags=%d, tag1=0x%x, tag2=0x%x, tage3=0x%x", inst_id, ch, num_tags, tag1, tag2, tag3);
    int addr = elb_mx_mac_t::ELB_MX_MAC_CHCONFIG8_ADDR0 + ch * elb_mx_mac_t::ELB_MX_MAC_CHCONFIG8_ADDR_INC;
    uint64_t data = elb_mx_apb_read(chip_id, inst_id, addr);
...
{
    MX_DEBUG_MSG("MX%d ch%d elb_mx_set_txpadrunt value = %d", inst_id, ch, value);
    int addr = elb_mx_mac_t::ELB_MX_MAC_MACCFG_ADDR0 + ch * elb_mx_mac_t::ELB_MX_MAC_MACCFG_ADDR_INC;
    uint64_t data = elb_mx_apb_read(chip_id, inst_id, addr);
...
{
    MX_DEBUG_MSG("MX%d ch%d elb_mx_set_tx_padding enable = %d", inst_id, ch, enable);
    if (enable == 1) {
...
elb_mx_set_rx_padding(int chip_id, int inst_id, int ch, int value) {
    MX_DEBUG_MSG("MX%d ch%d elb_mx_set_rx_padding value = %d", inst_id, ch, value);
    int addr = elb_mx_mac_t::ELB_MX_MAC_MACCFG_ADDR0 + ch * elb_mx_mac_t::ELB_MX_MAC_MACCFG_ADDR_INC;
    uint64_t data = elb_mx_apb_read(chip_id, inst_id, addr);
...
{
    MX_DEBUG_MSG("MX%d ch%d elb_mx_send_remote_faults send = %d", inst_id, ch, send);
    int addr = elb_mx_mac_t::ELB_MX_MAC_MACCFG_ADDR0 + ch * elb_mx_mac_t::ELB_MX_MAC_MACCFG_ADDR_INC;
    int value = (send) ? 1 : 0;
    uint64_t data = elb_mx_apb_read(chip_id, inst_id, addr);
...
{
    MX_DEBUG_MSG("MX%d ch%d elb_mx_set_hiser_thresh value = 0x%x", inst_id, ch, value);
...
elb_mx_enable_false_linkup_detection(int chip_id , int inst_id, int ch, int value) {
    MX_DEBUG_MSG("MX%d, ch%d: elb_mx_enable_false_linkup_detection", inst_id, ch);
    int addr = elb_mx_mac_t::ELB_MX_MAC_CHCONFIG3_ADDR0 + ch * elb_mx_mac_t::ELB_MX_MAC_CHCONFIG3_ADDR_INC;
    uint64_t data = elb_mx_apb_read(chip_id, inst_id, addr);
...
elb_mx_get_mac_chsts(int chip_id, int inst_id, int ch, bool display)
{
    ...
    if (display == 1) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            MX_DEBUG_MSG("MX%d ch%d chsts[%2d] = %d %s, field: %s", inst_id, ch, i, field_value, field_status.c_str(), elb_mx_mac_chsts_info[i*2+1].c_str());
        }
...
elb_mx_get_ch_fault(int chip_id, int inst_id, int ch)
{
    ...
    MX_DEBUG_MSG("MX%d ch%d elb_mx_get_ch_fault nolocalfault=%d, noremotefault=%d", inst_id, ch, (int) nolocalfault, (int) noremotefault);
...
elb_mx_get_ch_pcs_err(int chip_id, int inst_id, int ch)
{
    ...
    MX_DEBUG_MSG("MX%d ch%d elb_mx_get_ch_pcs_err nohiber=%d, hiser=%d", inst_id, ch, (int) nohiber, (int) hiser);
...
elb_mx_mac_stat(int chip_id, int inst_id, int ch, uint64_t *stats_data)  // For software
{
    for (int i = 0; i < ELB_MAX_MAC_STATS; i++) {
        ...
        //MX_DEBUG_MSG("MX%d ch%d %50s : %" PRIu64 "", inst_id, ch, elb_mx_mac_stat_info[i*3+2].c_str(), data);
    }
    if (stats_data != NULL) {
...
elb_mx_clear_mac_stat(int chip_id, int inst_id, int ch, int value) {
    MX_DEBUG_MSG("MX%d ch%d elb_mx_clear_mac_stat value = %d", inst_id, ch, value);
    int addr = elb_mx_mac_t::ELB_MX_MAC_MACCFG_ADDR0 + ch * elb_mx_mac_t::ELB_MX_MAC_MACCFG_ADDR_INC;
    uint64_t data = elb_mx_apb_read(chip_id, inst_id, addr);
...
elb_mx_get_mac_stat(int chip_id, int inst_id, int ch, int stat_type, bool skip_zero, bool clear_on_read, bool display)
{
    ...
    if (skip_zero && all_stats_zero) {
        ...
        MX_DEBUG_MSG("MX%d, ch%d: all %s statistic counters = 0", inst_id, ch, name.c_str());
    }
...
elb_mx_get_mac_int(int chip_id, int inst_id, int ch, bool clear_on_read, bool display)
{
    ...
    MX_DEBUG_MSG("elb_mx_get_mac_int, MX%d ch%d, clear_on_read = %d, data = 0x%x", inst_id, ch, clear_on_read, data);
    if (display && data != 0) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (field_value == 1) {
                MX_DEBUG_MSG("MX%d ch%d mac_intsts[%2d]=1 %s, field: %s", inst_id, ch, i, field_status.c_str(), elb_mx_mac_int_info[i*2+1].c_str());
            }
        }
    }
...
{
    MX_DEBUG_MSG("elb_mx_enable_mac_int, MX%d ch%d, vec = 0x%x", inst_id, ch, vec);
    int addr = elb_mx_mac_t::ELB_MX_MAC_INTCONTROL_ADDR0 + ch * elb_mx_mac_t::ELB_MX_MAC_INTCONTROL_ADDR_INC;
    uint64_t data = elb_mx_apb_read(chip_id, inst_id, addr);
...
{
    MX_DEBUG_MSG("elb_mx_clear_mac_int, MX%d ch%d, vec = 0x%x", inst_id, ch, vec);
    int addr = elb_mx_mac_t::ELB_MX_MAC_INTCONTROL_ADDR0 + ch * elb_mx_mac_t::ELB_MX_MAC_INTCONTROL_ADDR_INC;
    uint64_t data = elb_mx_apb_read(chip_id, inst_id, addr);
    // clear intsts because it is also interrupt override field
    elb_mx_set_field(&data, elb_mx_mac_t::ELB_MX_MAC_INTCONTROL_FLD_INTSTS_POS, elb_mx_mac_t::ELB_MX_MAC_INTCONTROL_FLD_INTSTS_SZ, 0);
...
elb_mx_get_an_int(int chip_id, int inst_id, int ch, bool clear_on_read, bool display)
{
    ...
    MX_DEBUG_MSG("elb_mx_get_an_int, MX%d ch%d, clear_on_read = %d, data = 0x%x", inst_id, ch, clear_on_read, data);
    if (display && data != 0) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (field_value == 1) {
                MX_DEBUG_MSG("MX%d ch%d anint[%2d]=1 %s, field: %s", inst_id, ch, i, field_status.c_str(), elb_mx_an_int_info[i*2+1].c_str());
            }
        }
    }
    if (clear_on_read && data != 0) {
...
{
    MX_DEBUG_MSG("elb_mx_enable_an_int, MX%d ch%d, vec = 0x%x", inst_id, ch, vec);
    int addr = elb_mx_an_t::ELB_MX_AN_ANINTCONTROL_ADDR0 + ch * elb_mx_an_t::ELB_MX_AN_ANINTCONTROL_ADDR_INC;
    uint64_t data = elb_mx_apb_read(chip_id, inst_id, addr);
    // clear intsts because it is also interrupt override field
...
{
    MX_DEBUG_MSG("elb_mx_clear_an_int, MX%d ch%d, vec = 0x%x", inst_id, ch, vec);
    int addr = elb_mx_an_t::ELB_MX_AN_ANINTCONTROL_ADDR0 + ch * elb_mx_an_t::ELB_MX_AN_ANINTCONTROL_ADDR_INC;
    uint64_t data = elb_mx_apb_read(chip_id, inst_id, addr);
    // clear intsts because it is also interrupt override field
    elb_mx_set_field(&data, elb_mx_an_t::ELB_MX_AN_ANINTCONTROL_FLD_INTSTS_POS, elb_mx_an_t::ELB_MX_AN_ANINTCONTROL_FLD_INTSTS_SZ, 0);
...
elb_mx_get_int_groups(int chip_id, int inst_id, bool display)
{
    ...
    MX_DEBUG_MSG("elb_mx_get_int_groups, MX%d data = 0x%x", inst_id, data);
    if (display && data != 0) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (field_value == 1) {
                MX_DEBUG_MSG("MX%d int_groups[%2d]=1 %s, field: %s", inst_id, i, field_status.c_str(), elb_mx_int_groups_info[i*2+1].c_str());
            }
        }
    }
    return data;
...
elb_mx_get_int_sd(int chip_id, int inst_id, bool clear_on_read, bool display)
{
    ...
    MX_DEBUG_MSG("elb_mx_get_int_sd, MX%d clear_on_read = %d, data = 0x%x", inst_id, clear_on_read, data);
    if (display && data != 0) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (field_value == 1) {
                MX_DEBUG_MSG("MX%d %10s[%2d]=1 %s, field: %s", inst_id, "int_sd", i, field_status.c_str(), elb_mx_int_sd_info[i*2+1].c_str());
            }
        }
    }
    if (clear_on_read && data != 0) {
...
{
    MX_DEBUG_MSG("elb_mx_enable_int_sd, MX%d vec = 0x%x", inst_id, vec);
    elb_mx_csr_t & mx_csr = PEN_BLK_REG_MODEL_ACCESS(elb_mx_csr_t, chip_id, inst_id);
...
{
    MX_DEBUG_MSG("elb_mx_disable_int_sd, MX%d vec = 0x%x", inst_id, vec);
    elb_mx_csr_t & mx_csr = PEN_BLK_REG_MODEL_ACCESS(elb_mx_csr_t, chip_id, inst_id);
...
{
    MX_DEBUG_MSG("elb_mx_clear_int_sd, MX%d vec = 0x%x", inst_id, vec);
    elb_mx_csr_t & mx_csr = PEN_BLK_REG_MODEL_ACCESS(elb_mx_csr_t, chip_id, inst_id);
...
elb_mx_get_int_mac(int chip_id, int inst_id, bool clear_on_read, bool display)
{
    ...
    MX_DEBUG_MSG("elb_mx_get_int_mac, MX%d clear_on_read = %d, data = 0x%x", inst_id, clear_on_read, data);
    if (display && data != 0) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (field_value == 1) {
                MX_DEBUG_MSG("MX%d %10s[%2d]=1 %s, field: %s", inst_id, "int_mac", i, field_status.c_str(), elb_mx_int_mac_info[i*2+1].c_str());
            }
        }
    }
    if (clear_on_read && data != 0) {
...
{
    MX_DEBUG_MSG("elb_mx_enable_int_mac, MX%d vec = 0x%x", inst_id, vec);
    elb_mx_csr_t & mx_csr = PEN_BLK_REG_MODEL_ACCESS(elb_mx_csr_t, chip_id, inst_id);
...
{
    MX_DEBUG_MSG("elb_mx_disable_int_mac, MX%d vec = 0x%x", inst_id, vec);
    elb_mx_csr_t & mx_csr = PEN_BLK_REG_MODEL_ACCESS(elb_mx_csr_t, chip_id, inst_id);
...
{
    MX_DEBUG_MSG("elb_mx_clear_int_mac, MX%d vec = 0x%x", inst_id, vec);
    elb_mx_csr_t & mx_csr = PEN_BLK_REG_MODEL_ACCESS(elb_mx_csr_t, chip_id, inst_id);
...
elb_mx_get_int_ecc(int chip_id, int inst_id, bool clear_on_read, bool display)
{
    ...
    MX_DEBUG_MSG("elb_mx_get_int_ecc, MX%d clear_on_read = %d, data = 0x%x", inst_id, clear_on_read, data);
    if (display && data != 0) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (field_value == 1) {
                MX_DEBUG_MSG("MX%d %10s[%2d]=1 %s, field: %s", inst_id, "int_ecc", i, field_status.c_str(), elb_mx_int_ecc_info[i*2+1].c_str());
            }
        }
    }
    if (clear_on_read && data != 0) {
...
{
    MX_DEBUG_MSG("elb_mx_enable_int_ecc, MX%d vec = 0x%x", inst_id, vec);
    elb_mx_csr_t & mx_csr = PEN_BLK_REG_MODEL_ACCESS(elb_mx_csr_t, chip_id, inst_id);
...
{
    MX_DEBUG_MSG("elb_mx_disable_int_ecc, MX%d vec = 0x%x", inst_id, vec);
    elb_mx_csr_t & mx_csr = PEN_BLK_REG_MODEL_ACCESS(elb_mx_csr_t, chip_id, inst_id);
...
{
    MX_DEBUG_MSG("elb_mx_clear_int_ecc, MX%d vec = 0x%x", inst_id, vec);
    elb_mx_csr_t & mx_csr = PEN_BLK_REG_MODEL_ACCESS(elb_mx_csr_t, chip_id, inst_id);
...
elb_mx_get_int_fixer(int chip_id, int inst_id, bool clear_on_read, bool display)
{
    ...
    MX_DEBUG_MSG("elb_mx_get_int_fixer, MX%d clear_on_read = %d, data = 0x%x", inst_id, clear_on_read, data);
    if (display && data != 0) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (field_value == 1) {
                MX_DEBUG_MSG("MX%d %10s[%2d]=1 %s, field: %s", inst_id, "int_fixer", i, field_status.c_str(), elb_mx_int_fixer_info[i*2+1].c_str());
            }
        }
    }
    if (clear_on_read && data != 0) {
...
{
    MX_DEBUG_MSG("elb_mx_enable_int_fixer, MX%d vec = 0x%x", inst_id, vec);
    elb_mx_csr_t & mx_csr = PEN_BLK_REG_MODEL_ACCESS(elb_mx_csr_t, chip_id, inst_id);
...
{
    MX_DEBUG_MSG("elb_mx_disable_int_fixer, MX%d vec = 0x%x", inst_id, vec);
    elb_mx_csr_t & mx_csr = PEN_BLK_REG_MODEL_ACCESS(elb_mx_csr_t, chip_id, inst_id);
...
{
    MX_DEBUG_MSG("elb_mx_clear_int_fixer, MX%d vec = 0x%x", inst_id, vec);
    elb_mx_csr_t & mx_csr = PEN_BLK_REG_MODEL_ACCESS(elb_mx_csr_t, chip_id, inst_id);
...
elb_mx_get_int_flush(int chip_id, int inst_id, bool clear_on_read, bool display)
{
    ...
    MX_DEBUG_MSG("elb_mx_get_int_flush, MX%d clear_on_read = %d, data = 0x%x", inst_id, clear_on_read, data);
    if (display && data != 0) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (field_value == 1) {
                MX_DEBUG_MSG("MX%d %10s[%2d]=1 %s, field: %s", inst_id, "int_flush", i, field_status.c_str(), elb_mx_int_flush_info[i*2+1].c_str());
            }
        }
    }
    if (clear_on_read && data != 0) {
...
{
    MX_DEBUG_MSG("elb_mx_enable_int_flush, MX%d vec = 0x%x", inst_id, vec);
    elb_mx_csr_t & mx_csr = PEN_BLK_REG_MODEL_ACCESS(elb_mx_csr_t, chip_id, inst_id);
...
{
    MX_DEBUG_MSG("elb_mx_disable_int_flush, MX%d vec = 0x%x", inst_id, vec);
    elb_mx_csr_t & mx_csr = PEN_BLK_REG_MODEL_ACCESS(elb_mx_csr_t, chip_id, inst_id);
...
{
    MX_DEBUG_MSG("elb_mx_clear_int_flush, MX%d vec = 0x%x", inst_id, vec);
    elb_mx_csr_t & mx_csr = PEN_BLK_REG_MODEL_ACCESS(elb_mx_csr_t, chip_id, inst_id);
...
elb_mx_dump_mx_cfg(int chip_id, int inst_id) {
    elb_mx_csr_t & mx_csr = PEN_BLK_REG_MODEL_ACCESS(elb_mx_csr_t, chip_id, inst_id);
    mx_csr.cfg_swrst.read();
    MX_DEBUG_MSG("MX%d elb_mx_show_mx_cfg cfg_swrst=%x", inst_id, mx_csr.cfg_swrst.all().convert_to<int>());
    mx_csr.cfg_mac.read();
    MX_DEBUG_MSG("MX%d elb_mx_show_mx_cfg cfg_mac=%x", inst_id, mx_csr.cfg_mac.all().convert_to<int>());
    mx_csr.cfg_mac_pfc_txxoff.read();
    MX_DEBUG_MSG("MX%d elb_mx_show_mx_cfg cfg_mac_pfc_txxoff=%x", inst_id, mx_csr.cfg_mac_pfc_txxoff.all().convert_to<int>());
    for (int i = 0; i < 5; i++) {
        mx_csr.cfg_port[i].read();
        MX_DEBUG_MSG("MX%d elb_mx_show_mx_cfg cfg_port[%d]=%x", inst_id, i, mx_csr.cfg_port[i].all().convert_to<int>());
        mx_csr.cfg_fixer[i].read();
        MX_DEBUG_MSG("MX%d elb_mx_show_mx_cfg cfg_fixer[%d]=%x", inst_id, i, mx_csr.cfg_fixer[i].all().convert_to<int>());
    }
...
elb_mx_dump_ch_cfg(int chip_id, int inst_id, int ch) {
    int addr = elb_mx_mac_t::ELB_MX_MAC_CHMODE_ADDR0 + ch * elb_mx_mac_t::ELB_MX_MAC_CHMODE_ADDR_INC;
    uint64_t data = elb_mx_apb_read(chip_id, inst_id, addr);
    MX_DEBUG_MSG("MX%d ch%d elb_mx_show_ch_cfg chmode=0x%" PRIx64 "", inst_id, ch, data);
    addr = elb_mx_mac_t::ELB_MX_MAC_MACCFG_ADDR0 + ch * elb_mx_mac_t::ELB_MX_MAC_MACCFG_ADDR_INC;
    data = elb_mx_apb_read(chip_id, inst_id, addr);
    MX_DEBUG_MSG("MX%d ch%d elb_mx_show_ch_cfg maccfg=0x%" PRIx64 "", inst_id, ch, data);
    addr = elb_mx_mac_t::ELB_MX_MAC_CHCONFIG3_ADDR0 + ch * elb_mx_mac_t::ELB_MX_MAC_CHCONFIG3_ADDR_INC;
    data = elb_mx_apb_read(chip_id, inst_id, addr);
    MX_DEBUG_MSG("MX%d ch%d elb_mx_show_ch_cfg chconfig3=0x%" PRIx64 "", inst_id, ch, data);
    addr = elb_mx_mac_t::ELB_MX_MAC_CHCONFIG4_ADDR0 + ch * elb_mx_mac_t::ELB_MX_MAC_CHCONFIG4_ADDR_INC;
    data = elb_mx_apb_read(chip_id, inst_id, addr);
    MX_DEBUG_MSG("MX%d ch%d elb_mx_show_ch_cfg chconfig4=0x%" PRIx64 "", inst_id, ch, data);
    addr = elb_mx_mac_t::ELB_MX_MAC_CHCONFIG5_ADDR0 + ch * elb_mx_mac_t::ELB_MX_MAC_CHCONFIG5_ADDR_INC;
    data = elb_mx_apb_read(chip_id, inst_id, addr);
    MX_DEBUG_MSG("MX%d ch%d elb_mx_show_ch_cfg chconfig5=0x%" PRIx64 "", inst_id, ch, data);
    addr = elb_mx_mac_t::ELB_MX_MAC_CHCONFIG6_ADDR0 + ch * elb_mx_mac_t::ELB_MX_MAC_CHCONFIG6_ADDR_INC;
    data = elb_mx_apb_read(chip_id, inst_id, addr);
    MX_DEBUG_MSG("MX%d ch%d elb_mx_show_ch_cfg chconfig6=0x%" PRIx64 "", inst_id, ch, data);
    addr = elb_mx_mac_t::ELB_MX_MAC_CHCONFIG7_ADDR0 + ch * elb_mx_mac_t::ELB_MX_MAC_CHCONFIG7_ADDR_INC;
    data = elb_mx_apb_read(chip_id, inst_id, addr);
    MX_DEBUG_MSG("MX%d ch%d elb_mx_show_ch_cfg chconfig7=0x%" PRIx64 "", inst_id, ch, data);
    addr = elb_mx_mac_t::ELB_MX_MAC_CHCONFIG8_ADDR0 + ch * elb_mx_mac_t::ELB_MX_MAC_CHCONFIG8_ADDR_INC;
    data = elb_mx_apb_read(chip_id, inst_id, addr);
    MX_DEBUG_MSG("MX%d ch%d elb_mx_show_ch_cfg chconfig8=0x%" PRIx64 "", inst_id, ch, data);
}
...
elb_mx_an_get_rx_base_page(int chip_id, int inst_id, int ch, bool display)
{
    ...
    if (display) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            MX_DEBUG_MSG("MX%d ch%d rx base_page[%2d] = %s = %d", inst_id, ch, elb_mx_an_base_config_t::ELB_MX_AN_BASE_PAUSE_CAPABILITY_POS+i, elb_mx_an_pause_info[i*2+1].c_str(), field_value);
        }
        ...
        MX_DEBUG_MSG("MX%d ch%d rx base_page[%2d] = np = %d", inst_id, ch, elb_mx_an_base_config_t::ELB_MX_AN_BASE_NP_POS, np);
        ...
        for (int i = 0; i < len; i++) {
            ...
            MX_DEBUG_MSG("MX%d ch%d rx base_page[%2d] = %s = %d", inst_id, ch, elb_mx_an_base_config_t::ELB_MX_AN_BASE_TECHABILITY_POS+i, elb_mx_an_techability_info[i*2+1].c_str(), field_value);
        }
        ...
        for (int i = 0; i < len; i++) {
            ...
            MX_DEBUG_MSG("MX%d ch%d rx base_page[%2d] = %s = %d", inst_id, ch, elb_mx_an_base_config_t::ELB_MX_AN_BASE_FEC_CAPABILITY_POS+i, elb_mx_an_fec_info[i*2+1].c_str(), field_value);
        }
    }
    return page0;
}
...
{
    MX_DEBUG_MSG("MX%d ch%d elb_mx_an_write_tx_page idx = %d, page = 0x%" PRIx64 "", inst_id, ch, idx, page);
...
{
    MX_DEBUG_MSG("MX%d ch%d elb_mx_an_reset value = %d", inst_id, ch, value);
...
{
    MX_DEBUG_MSG("MX%d ch%d elb_mx_an_enable value = %d", inst_id, ch, value);
    int addr = elb_mx_an_t::ELB_MX_AN_ANCFG_ADDR0 + ch * elb_mx_an_t::ELB_MX_AN_ANCFG_ADDR_INC;
    uint64_t data = elb_mx_apb_read(chip_id, inst_id, addr);
...
{
    MX_DEBUG_MSG("MX%d ch%d elb_mx_an_ignore_nonce value = %d", inst_id, ch, value);
    int addr = elb_mx_an_t::ELB_MX_AN_ANCFG_ADDR0 + ch * elb_mx_an_t::ELB_MX_AN_ANCFG_ADDR_INC;
    uint64_t data = elb_mx_apb_read(chip_id, inst_id, addr);
...
{
    MX_DEBUG_MSG("MX%d ch%d elb_mx_an_set_breaklinktimer value = 0x%" PRIx32 "", inst_id, ch, value);
...
{
    MX_DEBUG_MSG("MX%d ch%d elb_mx_an_init user_cap = 0x%x, user_fec = 0x%x, user_pause = %d, ignore_nonce = %d, breaklinktimer = 0x%x", inst_id, ch, user_cap, user_fec, user_pause, ignore_nonce, breaklinktimer);
...
elb_mx_get_ansts(int chip_id, int inst_id, int ch, bool display)
{
    ...
    if (display == 1) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            MX_DEBUG_MSG("MX%d ch%d ansts[%2d] = %d %s, field: %s", inst_id, ch, i, field_value, field_status.c_str(), elb_mx_an_ansts_info[i*2+1].c_str());
        }
...
elb_mx_an_get_an_complete(int chip_id, int inst_id, int ch)
{
    ...
    MX_DEBUG_MSG("MX%d ch%d elb_mx_an_get_an_complete ancomplete = %d, receivedlast = %d", inst_id, ch, ancomplete, receivedlast);
...
elb_mx_an_get_hcd(int chip_id, int inst_id, int ch)
{
    ...
    MX_DEBUG_MSG("MX%d ch%d elb_mx_an_get_hcd anmode = 0x%" PRIx64 ", hcd = 0x%" PRIx32 "", inst_id, ch, anmode, hcd);
    return hcd;
}
...
elb_mx_an_get_25g_fcfec(int chip_id, int inst_id, int ch)
{
    ...
    MX_DEBUG_MSG("MX%d ch%d elb_mx_an_get_25g_fcfec fecmode = 0x%" PRIx64 ", fcfec = %d", inst_id, ch, fecmode, fcfec);
    return fcfec;
}
...
elb_mx_an_get_25g_rsfec(int chip_id, int inst_id, int ch)
{
    ...
    MX_DEBUG_MSG("MX%d ch%d elb_mx_an_get_25g_rsfec fecmode = 0x%" PRIx64 ", rsfec = %d", inst_id, ch, fecmode, rsfec);
    return rsfec;
}
