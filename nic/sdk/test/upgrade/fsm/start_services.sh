...
if [ $# == 3 ]; then
    ...
    echo "usage : start_services.sh <bin_dir> <svc> <id>"
    echo "usage : start_services.sh <bin_dir> <svc> <id> <erro_code> <error_stage>"
    echo "usage : start_services.sh <bin_dir> <svc> <id> <erro_code> <error_stage> <time_delay>"
    exit 1
fi

CUR_DIR=$( readlink -f $( dirname $0 ))
TEST_BIN_NAME=upg_fsm_test
