...
config::get(const std::string &key) {
    ...
    if (update_ptree_()) {  // reread file if got updated since last read.
        data = this->prop_tree_.get<std::string>(key, "");
    } else {
        data = "";
    }
...
sdk_ret_t
config::set(const std::string &key, const std::string &data) {
    sdk_ret_t ret;
    struct stat sb;
    ...
    if (update_ptree_()) { // reread the file, if updated since last read.
        try {
            ...
            ret = SDK_RET_ERR;
            SDK_TRACE_ERR("Failed to write %s,%s to %s err %s",
                          key.c_str(), data.c_str(), this->conf_file_.c_str(),
                          ex.what());
        }
    } else {
        ret = SDK_RET_ERR;
