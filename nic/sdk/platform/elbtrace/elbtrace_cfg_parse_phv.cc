...
static inline void
sdptrace_json_phv_p4_parse(ptree &pt, elb_phv_intr_p4_c_t *cfg_inst,
                           elb_phv_intr_global_c_t cfg_inst_global)
{
    boost::optional<bool> val;
    boost::optional<std::string> str;
...
static inline void
sdptrace_json_phv_txdma_parse(ptree &pt, elb_phv_intr_txdma_c_t *cfg_inst,
                              elb_phv_intr_global_c_t cfg_inst_global)
{
    boost::optional<bool> val;
    boost::optional<std::string> str;
...
static inline void
sdptrace_json_phv_rxdma_parse(ptree &pt, elb_phv_intr_rxdma_c_t *cfg_inst,
                              elb_phv_intr_global_c_t cfg_inst_global)
{
    boost::optional<bool> val;
    boost::optional<std::string> str;
...
sdptrace_json_capture_parse_verify(sdptrace_cfg_inst_t *cfg_inst)
{
    ...
    if ((cfg_inst->pipeline_str == "p4ig") ||
        ...
        cout << "cfg_parse_phv_verif pipeline is  " << cfg_inst->pipeline_str
             << endl;
        ...
        //      (cfg_phv_txdma_data.padding_0>>64), (uint64_t)
        //      (cfg_phv_txdma_data.padding_0)); printf("txdma mask
        //      padding_0:0x%lx0x%lx\n", (uint64_t)
        //      (cfg_phv_txdma_mask.padding_0>>64), (uint64_t)
        ...
        //      cout << cfg_inst->pipeline_str << "is not a valid pipeline" <<
        //      endl;
    }
}
...
sdptrace_pack_global(cpp_int glb_data, elb_phv_intr_global_c_t *cfg_inst)
{
    ...
    glb_data =
        hlp.set_slc(glb_data, cfg_inst->tm_replicate_ptr, 40, 40 + 16 - 1);
...
static inline cpp_int
sdptrace_pack_p4(cpp_int p4_data, cpp_int glb_data,
                 elb_phv_intr_p4_c_t *cfg_inst)
{
    boost::optional<bool> val;
    boost::optional<std::string> str;
    ...
    p4_data =
        hlp.set_slc(p4_data, cfg_inst->deparser_csum_disable, 327, 327 + 7 - 1);
    p4_data =
        hlp.set_slc(p4_data, cfg_inst->deparser_crc_disable, 326, 326 + 1 - 1);
...
static inline cpp_int
sdptrace_pack_tx(cpp_int tx_data, cpp_int glb_data,
                 elb_phv_intr_txdma_c_t *cfg_inst)
{
    boost::optional<bool> val;
    boost::optional<std::string> str;
...
static inline cpp_int
sdptrace_pack_rx(cpp_int rx_data, cpp_int glb_data,
                 elb_phv_intr_rxdma_c_t *cfg_inst)
{
    boost::optional<bool> val;
    boost::optional<std::string> str;
    ...
    rx_data =
        hlp.set_slc(rx_data, cfg_inst->rx_splitter_offset, 259, 259 + 10 - 1);
...
sdptrace_json_capture_parse(ptree &pt, sdptrace_cfg_inst_t *cfg_inst)
{
    ...
    for (ptree::iterator pos = pt.begin(); pos != pt.end();) {
        if (strcmp(pos->first.data(), "trigger_data") == 0) {
            ...
            sdptrace_json_phv_p4_parse(pos->second, &cfg_phv_p4_data,
                                       cfg_phv_global_data);
            sdptrace_json_phv_txdma_parse(pos->second, &cfg_phv_txdma_data,
                                          cfg_phv_global_data);
            sdptrace_json_phv_rxdma_parse(pos->second, &cfg_phv_rxdma_data,
                                          cfg_phv_global_data);
        } else if (strcmp(pos->first.data(), "trigger_mask") == 0) {
            sdptrace_json_phv_global_parse(pos->second, &cfg_phv_global_mask);
            sdptrace_json_phv_p4_parse(pos->second, &cfg_phv_p4_mask,
                                       cfg_phv_global_mask);
            sdptrace_json_phv_txdma_parse(pos->second, &cfg_phv_txdma_mask,
                                          cfg_phv_global_mask);
            sdptrace_json_phv_rxdma_parse(pos->second, &cfg_phv_rxdma_mask,
                                          cfg_phv_global_mask);
        }
        pos++;
    }
    ...
    glb_data_1 =
        sdptrace_pack_global(glb_data_1, &cfg_phv_txdma_data.phv_global);
    hdr_data_1 = sdptrace_pack_tx(hdr_data_1, glb_data_1, &cfg_phv_txdma_data);
    glb_mask_1 =
        sdptrace_pack_global(glb_mask_1, &cfg_phv_txdma_mask.phv_global);
    hdr_mask_1 = sdptrace_pack_tx(hdr_mask_1, glb_mask_1, &cfg_phv_txdma_mask);

    glb_data_2 =
        sdptrace_pack_global(glb_data_2, &cfg_phv_rxdma_data.phv_global);
    hdr_data_2 = sdptrace_pack_rx(hdr_data_2, glb_data_2, &cfg_phv_rxdma_data);
    glb_mask_2 =
        sdptrace_pack_global(glb_mask_2, &cfg_phv_rxdma_mask.phv_global);
    ...
    //      cout << "parse phv trigger data " << hex <<
    //      cfg_inst->capture.trigger_data << endl; cout << "parse phv trigger
    //      mask " << hex << cfg_inst->capture.trigger_mask << endl;
