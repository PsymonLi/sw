    ...
    mem_addr_t ctl_base_addr, bool enable)
{
    ...
    sdp_axi.ring_size((uint32_t)((cfg_inst->settings.ring_size) >>
                                 4));    // todo: check with Neil/pratima
    ...
    if (cfg_inst->pipeline_str == "rxdma") {
        trigger_data_val = hlp0.set_slc(
            trigger_data_val, cfg_inst->capture.trigger_data_rx, 0, 511);
    } else if (cfg_inst->pipeline_str == "txdma") {
        trigger_data_val = hlp0.set_slc(
            trigger_data_val, cfg_inst->capture.trigger_data_tx, 0, 511);
    } else {
        trigger_data_val = hlp0.set_slc(
            trigger_data_val, cfg_inst->capture.trigger_data_p4, 0, 511);
    }
    ...
    if (cfg_inst->pipeline_str == "rxdma") {
        trigger_mask_val = hlp1.set_slc(
            trigger_mask_val, cfg_inst->capture.trigger_mask_rx, 0, 511);
    } else if (cfg_inst->pipeline_str == "txdma") {
        trigger_mask_val = hlp1.set_slc(
            trigger_mask_val, cfg_inst->capture.trigger_mask_rx, 0, 511);
    } else {
        trigger_mask_val = hlp1.set_slc(
            trigger_mask_val, cfg_inst->capture.trigger_mask_p4, 0, 511);
    }
...
{
    static mem_addr_t trace_addr = g_state.mputrace_base;
                   ...
                   std::string mod_name)
{
    ...
    dmatrace_dump_trace_hdr_fill(&trace_hdr, elb0, pipeline, mod_name);
    fwrite(&trace_hdr, sizeof(uint8_t), sizeof(trace_hdr), fp);
}
...
static inline void
sdptrace_dump_trace_info_write(elb_sdp_csr_cfg_sdp_axi_t sdp_axi, FILE *fp)
{
    uint8_t buf[64] = { 0 };
...
static inline void
dmatrace_dump_trace_info_write(elb_top_csr_t &elb0, FILE *fp,
                               std::string mod_name)
{
    uint8_t buf[64] = { 0 };
                               ...
                               int pipeline, int stage, int mpu)
{
    ...
    mputrace_dump_trace_hdr_write(trace, watch_pc, watch_data, pipeline, stage,
                                  mpu, fp);
    mputrace_dump_trace_info_write(trace, fp);
}

static inline void
sdptrace_dump_trace_file_write(
    elb_sdp_csr_sta_sdp_axi_write_control_t ctl_ptr,
    elb_sdp_csr_sta_sdp_axi_write_phv_t phv_ptr,
    elb_sdp_csr_cfg_sdp_axi_t sdp_axi,
    elb_sdp_csr_cfg_sdp_axi_sw_reset_t &sdp_axi_sw_reset,
    elb_sdp_csr_cfg_sdp_trace_trigger_t &sdp_trace_trigger, int pipeline,
    int stage)
{
    static FILE *fp = fopen(g_state.ELBTRACE_DUMP_FILE, "wb");
    ...
    sdptrace_dump_trace_hdr_write(ctl_ptr, phv_ptr, sdp_axi, sdp_axi_sw_reset,
                                  sdp_trace_trigger, pipeline, stage, fp);
    sdptrace_dump_trace_info_write(sdp_axi, fp);
}

static inline void
dmatrace_dump_trace_file_write(elb_top_csr_t &elb0, int pipeline,
                               std::string mod_name)
{
    static FILE *fp = fopen(g_state.ELBTRACE_DUMP_FILE, "wb");
    ...
    dmatrace_dump_trace_hdr_write(elb0, pipeline, fp, mod_name);
    dmatrace_dump_trace_info_write(elb0, fp, mod_name);
}
                                  ...
                                  int pipeline)
{
    for (int stage = 0; stage < stage_count; stage++) {
        for (int mpu = 0; mpu < ELBTRACE_MAX_MPU; mpu++) {
            if (mputrace_util_is_enabled(stg_ptr[stage].mpu[mpu].trace,
                                         stg_ptr[stage].mpu[mpu].watch_pc,
                                         stg_ptr[stage].mpu[mpu].watch_data)) {
                mputrace_dump_trace_file_write(
                    stg_ptr[stage].mpu[mpu].trace,
                    stg_ptr[stage].mpu[mpu].watch_pc,
                    stg_ptr[stage].mpu[mpu].watch_data, pipeline, stage, mpu);
            }
        }
        //        stg_ptr++;
    }
...
void
elbtrace_dump(const char *dump_file, std::string mod_name)
{
                        ...
                        mputrace_cfg_inst_t *cfg_inst)
{
    for (int s = 0; s < stage_count; s++) {
        for (int mpu = 0; mpu < ELBTRACE_MAX_MPU; mpu++) {
            ...
            mputrace_util_reg_program(
                stg_ptr[s].mpu[mpu].trace, stg_ptr[s].mpu[mpu].watch_pc,
                stg_ptr[s].mpu[mpu].watch_data, cfg_inst, 0, false);
            //        printf("%10" PRIu32
            //" %10" PRIu32 "\n",
            //   s,
            // mpu);
...
sdptrace_reset_buffer_erase(elb_sdp_csr_cfg_sdp_axi_t sdp_axi)
{
    ...
    if ((int)sdp_axi.int_var__enable) {
        ...
        sdk::lib::pal_mem_set(phv_base_addr, 0,
                              ring_size * SDPTRACE_PHV_ENTRY_SIZE);
        sdk::lib::pal_mem_set(ctl_base_addr, 0,
                              ring_size * SDPTRACE_CTL_ENTRY_SIZE);
    }
}
                        ...
                        sdptrace_cfg_inst_t *cfg_inst)
{
    for (int s = 0; s < stage_count; s++) {
        ...
        sdptrace_util_reg_program(
            stg_ptr[s].sdp.cfg_sdp_axi, stg_ptr[s].sdp.cfg_sdp_axi_sw_reset,
            stg_ptr[s].sdp.cfg_sdp_trace_trigger, cfg_inst, 0, 0, false);
...
static inline void
dmatrace_reset_buffer_erase(elb_top_csr_t &elb0, std::string mod_name)
{
...
static inline void
dmatrace_reset_pipeline(elb_top_csr_t &elb0, dmatrace_cfg_inst_t *cfg_inst,
                        std::string mod_name)
{
                                  ...
                                  int mpu, int pipeline)
{
    ...
    if (mputrace_util_is_enabled(trace, watch_pc, watch_data)) {
        printf("%10s"
               ...
               " %10" PRIu8 " %10" PRIu8 " %10" PRIu8 " %10" PRIu8 " %10" PRIu8
               "  "
               " 0x%08" PRIx64 " %8" PRIu32 " %10" PRIu32 "\n",
               // selectors
               mputrace_pipeline_str_get(pipeline).c_str(), stage, mpu,
               // ctrl
                                  ...
                                  int mpu, int pipeline)
{
    ...
    if (mputrace_util_is_enabled(trace, watch_pc, watch_data)) {
        printf("%10s"
               ...
               mputrace_pipeline_str_get(pipeline).c_str(), stage, mpu,
               // WATCH_PC
               (uint32_t)watch_pc.trace(), (uint32_t)watch_pc.count(),
               (uint32_t)watch_pc.intr(), (uint32_t)watch_pc.stop(),
                                  ...
                                  int mpu, int pipeline)
{
    ...
    if (mputrace_util_is_enabled(trace, watch_pc, watch_data)) {
        printf("%10s"
               ...
               mputrace_pipeline_str_get(pipeline).c_str(), stage, mpu,
               /// WATCH_DATA
               (uint32_t)watch_data.rtrace(), (uint32_t)watch_data.wtrace(),
               (uint32_t)watch_data.rintr(), (uint32_t)watch_data.wintr(),
               (uint32_t)watch_data.rstop(), (uint32_t)watch_data.wstop(),
               (uint32_t)watch_data.rexception(),
               (uint32_t)watch_data.wexception(),
               (uint64_t)watch_data.addr_lo(), (uint64_t)watch_data.addr_hi());
    };
}
                         ...
                         mputrace_cfg_inst_t *cfg_inst, int pipeline)
{
    ...
    for (int stage = 0; stage < stage_count; stage++) {
        for (int mpu = 0; mpu < ELBTRACE_MAX_MPU; mpu++) {
            mputrace_show_pipeline_internal_0(
                stg_ptr[stage].mpu[mpu].trace, stg_ptr[stage].mpu[mpu].watch_pc,
                stg_ptr[stage].mpu[mpu].watch_data, cfg_inst, stage, mpu,
                pipeline);
        }
    }
}
                         ...
                         mputrace_cfg_inst_t *cfg_inst, int pipeline)
{
    ...
    for (int stage = 0; stage < stage_count; stage++) {
        for (int mpu = 0; mpu < ELBTRACE_MAX_MPU; mpu++) {
            mputrace_show_pipeline_internal_1(
                stg_ptr[stage].mpu[mpu].trace, stg_ptr[stage].mpu[mpu].watch_pc,
                stg_ptr[stage].mpu[mpu].watch_data, cfg_inst, stage, mpu,
                pipeline);
        }
                         ...
                         mputrace_cfg_inst_t *cfg_inst, int pipeline)
{
    ...
    for (int stage = 0; stage < stage_count; stage++) {
        for (int mpu = 0; mpu < ELBTRACE_MAX_MPU; mpu++) {
            mputrace_show_pipeline_internal_2(
                stg_ptr[stage].mpu[mpu].trace, stg_ptr[stage].mpu[mpu].watch_pc,
                stg_ptr[stage].mpu[mpu].watch_data, cfg_inst, stage, mpu,
                pipeline);
        }
...
mputrace_show_all_pipelines(void)
{
    ...
    printf("%10s %10s %10s %10s %10s %10s %10s %10s %10s %10s "
           ...
           "phv_debug", "phv_error", "table_kd", "instr", "trace_addr",
           "trace_nent", "trace_sz");

    ELBTRACE_FOR_EACH_PIPELINE_SHOW(elb0.sgi, mputrace_show_pipeline_0,
                                    &cfg_inst, 0);
    ELBTRACE_FOR_EACH_PIPELINE_SHOW(elb0.sge, mputrace_show_pipeline_0,
                                    &cfg_inst, 1);
    ELBTRACE_FOR_EACH_PIPELINE_SHOW(elb0.pcr, mputrace_show_pipeline_0,
                                    &cfg_inst, 2);
    ELBTRACE_FOR_EACH_PIPELINE_SHOW(elb0.pct, mputrace_show_pipeline_0,
                                    &cfg_inst, 3);
    ELBTRACE_FOR_EACH_PIPELINE_SHOW(elb0.xg, mputrace_show_pipeline_0,
                                    &cfg_inst, 4);

    printf("\n%10s %10s %10s %10s %10s %10s %10s %10s %11s %11s\n", "pipeline",
           "stage", "mpu", "wpc_trace", "wpc_count", "wpc_intr", "wpc_stop",
           "wpc_exception", "wpc_addr_lo", "wpc_addr_hi");

    ELBTRACE_FOR_EACH_PIPELINE_SHOW(elb0.sgi, mputrace_show_pipeline_1,
                                    &cfg_inst, 0);
    ELBTRACE_FOR_EACH_PIPELINE_SHOW(elb0.sge, mputrace_show_pipeline_1,
                                    &cfg_inst, 1);
    ELBTRACE_FOR_EACH_PIPELINE_SHOW(elb0.pcr, mputrace_show_pipeline_1,
                                    &cfg_inst, 2);
    ELBTRACE_FOR_EACH_PIPELINE_SHOW(elb0.pct, mputrace_show_pipeline_1,
                                    &cfg_inst, 3);
    ELBTRACE_FOR_EACH_PIPELINE_SHOW(elb0.xg, mputrace_show_pipeline_1,
                                    &cfg_inst, 4);

    printf(
        "\n%10s %10s %10s %10s %10s %10s %10s %10s %10s %10s %10s %10s %10s\n",
        "pipeline", "stage", "mpu", "wd_rtrace", "wd_wtrace", "wd_rintr",
        "wd_wintr", "wd_rstop", "wd_wstop", "wd_rexception", "wd_wexception",
        "wd_addr_lo", "wd_addr_hi");

    ELBTRACE_FOR_EACH_PIPELINE_SHOW(elb0.sgi, mputrace_show_pipeline_2,
                                    &cfg_inst, 0);
    ELBTRACE_FOR_EACH_PIPELINE_SHOW(elb0.sge, mputrace_show_pipeline_2,
                                    &cfg_inst, 1);
    ELBTRACE_FOR_EACH_PIPELINE_SHOW(elb0.pcr, mputrace_show_pipeline_2,
                                    &cfg_inst, 2);
    ELBTRACE_FOR_EACH_PIPELINE_SHOW(elb0.pct, mputrace_show_pipeline_2,
                                    &cfg_inst, 3);
    ELBTRACE_FOR_EACH_PIPELINE_SHOW(elb0.xg, mputrace_show_pipeline_2,
                                    &cfg_inst, 4);
}
...
static inline void
sdptrace_show_pipeline_internal(
    elb_sdp_csr_cfg_sdp_axi_t &sdp_axi,
    elb_sdp_csr_cfg_sdp_axi_sw_reset_t &sdp_axi_sw_reset,
    elb_sdp_csr_cfg_sdp_trace_trigger_t &sdp_trace_trigger,
    sdptrace_cfg_inst_t *cfg_inst, int stage)
{
    ...
    if ((int)sdp_axi.int_var__enable) {
        printf("%10s"
               "%10" PRIu32 " %10" PRIu8 " %10" PRIu8 " %10" PRIu8 " %12" PRIu8
               " %18" PRIu8
               //           " 0x%08" PRIx512    todo
               //           " 0x%08" PRIx512    todo
               "        "
                       ...
                       sdptrace_cfg_inst_t *cfg_inst)
{
    ...
    for (int stage = 0; stage < stage_count; stage++) {
        sdptrace_show_pipeline_internal(
            stg_ptr[stage].sdp.cfg_sdp_axi,
            stg_ptr[stage].sdp.cfg_sdp_axi_sw_reset,
            stg_ptr[stage].sdp.cfg_sdp_trace_trigger, cfg_inst, stage);
    }
}
...
sdptrace_show(void)
{
    ...
    printf("%10s %10s %10s %10s %10s %10s %10s %10s "
           "%10s %10s\n",
           "pipeline", "stage", "enable", "trace_trig_en", "sw_reset_en",
           "no_trace_when_full", "stop_when_full", "phv_base_addr",
           "ctl_base_addr", "ring_sz");

    sdptrace_show_all_pipelines();

    //   cout << "size of  sdptrace header is " << dec <<
    //   sizeof(sdptrace_trace_hdr_t) << endl;
...
static inline void
dmatrace_show_pipeline_internal(elb_top_csr_t &elb0, std::string mod_name)
{
    elb_prd_csr_cfg_trace_t prd_trace = elb0.pr.pr.prd.cfg_trace;
    elb_ptd_csr_cfg_trace_t ptd_trace = elb0.pt.pt.ptd.cfg_trace;
    elb_ptd_csr_cfg_trace_t xd_trace = elb0.xd.pt.ptd.cfg_trace;
    ...
    if (mod_name == "prd") {
        if ((int)prd_trace.int_var__enable) {
            printf("%10s"
                   "%10" PRIu32 " %10" PRIu8 " %10" PRIu8 " %10" PRIu8
                   " %15" PRIu8 " %23" PRIu8 " %10" PRIu8 "  "
                   " 0x%08" PRIx64 " %7" PRIu32 "\n",
                   "prd", (uint32_t)prd_trace.enable(),
                   (uint32_t)prd_trace.phv_enable(),
                   (uint32_t)prd_trace.capture_all(),
                   (uint32_t)prd_trace.axi_err_enable(),
                   (uint32_t)prd_trace.pkt_phv_sync_err_enable(),
                   (uint32_t)prd_trace.wrap(), (uint32_t)prd_trace.rst(),
                   (long unsigned int)(prd_trace.base_addr() << 6),
                   (uint32_t)(prd_trace.buf_size()));
            //               (uint32_t)(ELBTRACE_ONE <<
            //               (uint32_t)prd_trace.buf_size()));
        }
        ...
        if ((int)prd_trace.int_var__enable) {
            printf("%10s"
                   "%10" PRIu32 " %10" PRIu8 " %10" PRIu8 " %10" PRIu8 " %15s"
                   " %23" PRIu8 " %10" PRIu8 "  "
                   " 0x%08" PRIx64 " %7" PRIu32 "\n",
                   "ptd", (uint32_t)ptd_trace.enable(),
                   (uint32_t)ptd_trace.phv_enable(),
                   (uint32_t)ptd_trace.capture_all(),
                   (uint32_t)ptd_trace.axi_err_enable(), "NA",
                   (uint32_t)ptd_trace.wrap(), (uint32_t)ptd_trace.rst(),
        ...
        if ((int)xd_trace.int_var__enable) {
            printf("%10s"
                   ...
                   " %23" PRIu8 " %10" PRIu8 "  "
                   " 0x%08" PRIx64 " %7" PRIu32 "\n",
                   "xd", (uint32_t)xd_trace.enable(),
                   (uint32_t)xd_trace.phv_enable(),
                   (uint32_t)xd_trace.capture_all(),
                   (uint32_t)xd_trace.axi_err_enable(), "NA",
                   (uint32_t)xd_trace.wrap(), (uint32_t)xd_trace.rst(),
...
dmatrace_show(void)
{
    ...
    printf("%10s %10s %10s %10s %10s %10s %10s %10s %10s %10s\n", "pipeline",
           "enable", "phv_enable", "capture_all", "axi_err_enable",
           "pkt_phv_sync_err_enable", "wrap", "reset", "trace_addr", "buf_sz");
