...
def copy_mod(dst):
    with open('/usr/src/github.com/pensando/sw/nic/athena/apps/athena_app/Makefile', 'r') as file:
...
def copy_includes (dest, arch):
    ...
    copytree("build/{0}/athena/{1}/out/dpdk_submake/include".format(arch, asic),
             "{0}/nic/sdk/dpdk/build/include".format(dest))
    #nic/include and nic/sdk/include have some dead symlinks
    try:
        copytree("include", "{0}/nic/include".format(dest))
    except :
         pass
    try:
        copytree("sdk/include", "{0}/nic/sdk/include".format(dest))
    except :
         pass
    copytree("athena/api/impl",
             "{0}/nic/athena/api/impl".format(dest),
             ignore=ignore_patterns("*.cc", "*.mk", "Makefile"))
    copytree("athena/api/include", "{0}/nic/athena/api/include".format(dest))
    ...
    copytree("athena/test/api/utils",
             ...
             ignore=ignore_patterns("*.cc", "*.mk", "Makefile"))
    copytree("athena/test/base", "{0}/nic/athena/test/base".format(dest),
             ignore=ignore_patterns("*.cc", "*.mk", "Makefile"))
