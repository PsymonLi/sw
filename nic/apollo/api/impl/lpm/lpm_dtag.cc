...
#include <math.h>
#include "lpm_dtag.hpp"
#include "nic/apollo/framework/impl_base.hpp"
#include "nic/apollo/framework/pipeline_impl_base.hpp"
#include "gen/p4gen/p4plus_rxdma/include/p4plus_rxdma_p4pd.h"

sdk_ret_t
lpm_dtag_add_key_to_stage(uint8_t *bytes, uint32_t idx, lpm_inode_t *lpm_inode)
{
...
sdk_ret_t
lpm_dtag_add_key_to_last_stage(uint8_t *bytes, uint32_t idx, lpm_inode_t *lpm_inode)
{
    auto table = (rxlpm2_actiondata_t *)bytes;
...
sdk_ret_t
lpm_dtag_set_default_data (uint8_t *bytes, uint32_t default_data)
{
    auto table = (rxlpm2_actiondata_t *) bytes;
...
sdk_ret_t
lpm_dtag_write_stage_table (mem_addr_t addr, uint8_t *bytes)
{
...
sdk_ret_t
lpm_dtag_write_last_stage_table (mem_addr_t addr, uint8_t *bytes)
{
    return impl_base::pipeline_impl()->write_to_rxdma_table(addr,
               P4_P4PLUS_RXDMA_TBL_ID_RXLPM2,
...
/**
 * key size is 4 bytes for dtag
 */
uint32_t
lpm_dtag_key_size (void)
{
 ...
 *
 * The computation is done as follows for DTAG :
 ...
 */
uint32_t
lpm_dtag_stages (uint32_t num_intrvls)
{
