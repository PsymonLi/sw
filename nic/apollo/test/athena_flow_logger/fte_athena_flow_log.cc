...
{   
    ...
    if (key->key_type == KEY_TYPE_IPV6) {
        ...
        inet_ntop(AF_INET, key->ip_saddr, srcstr, INET_ADDRSTRLEN);
        inet_ntop(AF_INET, key->ip_daddr, dststr, INET_ADDRSTRLEN);
    } else {
        PDS_TRACE_ERR("Unsupported Key type %d. Return\n", key->key_type);
        return;
    }
    strftime(start_time, sizeof(start_time), "%x - %I:%M:%S%p", 
             localtime(&data->start_time));
    strftime(end_time, sizeof(end_time), "%x - %I:%M:%S%p", 
             localtime(&data->last_time));
    fprintf(fp, "Index %u:\n" 
            "Key ==> SrcIP: %s, DstIP: %s, Dport: %u, Sport: %u, Proto %u, "
            ...
            arg->key.entry_idx, srcstr, dststr, key->dport, key->sport,
