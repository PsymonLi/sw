                 ...
                 action_t action, uint32_t priority, uint32_t base_rule_id)
{
    ...
    // and rest of rules as stateful-less
    ...
    if (is_l3(layer)) {
        ...
        // create half of stateful rules as icmp rules
        // and other halves as tcp rules, same for stateless rule as well
        ...
        // generate rules of all combinations of src & dst : range, prefix and 
        // tag
        src_index = (rule_id / NUM_L3_MATCH_TYPES) % NUM_L3_MATCH_TYPES;
        dst_index = (rule_id % NUM_L3_MATCH_TYPES);
        // as match types start with index 1
        switch (src_index + 1) {
        ...
        case IP_MATCH_RANGE:
            ...
            break;
        case IP_MATCH_TAG:
            attrs->match.l3_match.src_match_type = IP_MATCH_TAG;
            attrs->match.l3_match.src_tag = 1;
            break;
        }
        ...
        case IP_MATCH_RANGE:
            ...
            break;
        case IP_MATCH_TAG:
            attrs->match.l3_match.dst_match_type = IP_MATCH_TAG;
            attrs->match.l3_match.dst_tag = 2; 
            break;
        }
   }
    if (is_l4(layer)) {
...
policy_rule_feeder::init(uint32_t rule_id, uint32_t policy_id,
    ...
    num_obj = num_rules;
    test::extract_ip_pfx((char *)cidr_str.c_str(), &this->base_ip_pfx);
    fill_rule_attrs(&this->spec.attrs, this->base_ip_pfx, rule_id, num_obj,
...
policy_rule_feeder::iter_next(int width) {
    ...
    spec.key.rule_id = int2pdsobjkey(rule_id);
    cur_iter_pos++;
    test::increment_ip_addr(&this->base_ip_pfx.addr);
    fill_rule_attrs(&spec.attrs, this->base_ip_pfx, rule_id, num_obj,
                    LAYER_ALL, ALLOW, 0, this->base_rule_id);
}
