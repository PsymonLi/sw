...
TEST_F(flow_cache_scale_profile, DISABLED_create1M) {
    pds_ret_t rs;
    rs = create_entries(1024*1024, PDS_RET_OK);
    ASSERT_TRUE(rs == PDS_RET_OK);
}

TEST_F(flow_cache_scale_profile, DISABLED_delete1M) {
    pds_ret_t rs;
    rs = create_entries(1024*1024, PDS_RET_OK);
    ASSERT_TRUE(rs == PDS_RET_OK);
...
TEST_F(flow_cache_scale_profile, create2M) {
    pds_ret_t rs;
    rs = create_entries(2*1024*1024, PDS_RET_OK);
    ASSERT_TRUE(rs == PDS_RET_OK);
}

TEST_F(flow_cache_scale_profile, DISABLED_delete2M) {
    pds_ret_t rs;
    rs = create_entries(2*1024*1024, PDS_RET_OK);
    ASSERT_TRUE(rs == PDS_RET_OK);
...
TEST_F(flow_cache_scale_profile, DISABLED_create4M) {
    pds_ret_t rs;
    rs = create_entries(4*1024*1024, PDS_RET_OK);
    ASSERT_TRUE(rs == PDS_RET_OK);
}

TEST_F(flow_cache_scale_profile, DISABLED_delete4M) {
    pds_ret_t rs;
    rs = create_entries(4*1024*1024, PDS_RET_OK);
    ASSERT_TRUE(rs == PDS_RET_OK);
