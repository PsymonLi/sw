...
func (ms *MbusServer) IsObjectValidForDSC(dsc, kind string, ometa api.ObjectMeta) bool {
	return ms.memDB.IsObjectValidForDSC(dsc, kind, ometa)
}

// SetPropagationStatusChannel is used to the propagationstatusupdate channel from statemgr
func (ms *MbusServer) SetPropagationStatusChannel(c chan *memdb.PropagationStTopoUpdate) {
...
func (eh *AggregateTopic) ListObjects(ctx context.Context, kinds *api.AggWatchOptions) (*netproto.AggObjectList, error) {
	...
	for _, kind := range kinds.WatchOptions {
		...
		case "VirtualRouterPeeringGroup":
			...
			if _, ok := eh.statusReactor.(VirtualRouterPeeringGroupStatusReactor); ok {
				filters = eh.statusReactor.(VirtualRouterPeeringGroupStatusReactor).GetAgentWatchFilter(ctx, kind.Group+"."+kind.Kind, &kind.Options)
			}

			objlist, err := eh.server.ListVirtualRouterPeeringGroups(context.Background(), nodeID, filters)
			if err != nil {
				log.Errorf("Error getting a list of objects. Err: %v", err)
				return nil, err
			}
...
func (eh *AggregateTopic) ObjectOperUpdate(stream netproto.AggWatchApiV1_ObjectOperUpdateServer) error {
	...
	for {
		...
		case api.EventType_UpdateEvent:
			...
			case "VirtualRouterPeeringGroup":
				if _, ok := eh.statusReactor.(VirtualRouterPeeringGroupStatusReactor); ok {
					err = eh.statusReactor.(VirtualRouterPeeringGroupStatusReactor).OnVirtualRouterPeeringGroupOperUpdate(nodeID,
						object.Message.(*netproto.VirtualRouterPeeringGroup))
					if err != nil {
						log.Errorf("Error updating VirtualRouterPeeringGroup oper state. Err: %v", err)
					}
				}
				eh.updateAckedObjStatus(nodeID, oper.AggObj.Kind, oper.EventType, object.Message.(*netproto.VirtualRouterPeeringGroup).GetObjectMeta())
		...
		case api.EventType_DeleteEvent:
			...
			case "VirtualRouterPeeringGroup":
				if _, ok := eh.statusReactor.(VirtualRouterPeeringGroupStatusReactor); ok {
					err = eh.statusReactor.(VirtualRouterPeeringGroupStatusReactor).OnVirtualRouterPeeringGroupOperDelete(nodeID,
						object.Message.(*netproto.VirtualRouterPeeringGroup))
					if err != nil {
						log.Errorf("Error updating VirtualRouterPeeringGroup oper state. Err: %v", err)
					}
...
func (eh *AggregateTopic) handleReconcileEvent(aggKey string, ctx context.Context, evt memdb.Event, stream netproto.AggWatchApiV1_WatchObjectsServer) {
	nodeID := netutils.GetNodeUUIDFromCtx(ctx)
	...
	case "VirtualRouterPeeringGroup":
		objlist, err := eh.server.ListVirtualRouterPeeringGroupsNoFilter(context.Background())
		if err != nil {
			log.Errorf("Error getting a list of objects. Err: %v", err)
			return
		}
...
func (eh *AggregateTopic) WatchObjects(kinds *api.AggWatchOptions, stream netproto.AggWatchApiV1_WatchObjectsServer) error {
	...
	for _, kind := range kinds.WatchOptions {
		...
		case "VirtualRouterPeeringGroup":
			if _, ok := eh.statusReactor.(VirtualRouterPeeringGroupStatusReactor); ok {
				watcher.Filters[kind.Kind] = eh.statusReactor.(VirtualRouterPeeringGroupStatusReactor).GetAgentWatchFilter(ctx, kind.Group+"."+kind.Kind, &kind.Options)
			} else {
				filt := func(obj, prev memdb.Object) bool {
					return true
				}
	...
	for _, kind := range kindStrings {
		...
		case "VirtualRouterPeeringGroup":
			objlist, err := eh.server.ListVirtualRouterPeeringGroups(context.Background(), nodeID, watcher.Filters[kind])
			if err != nil {
				log.Errorf("Error getting a list of objects. Err: %v", err)
				return err
			}
	...
	for {
		...
		case evt, ok := <-watcher.Channel:
			...
			if evt.EventType == memdb.ReconcileEvent {
				...
				// send any pending events before processing the reconcile event
				if len(watchEvts.AggObjectEvents) > 0 {
				...
				eh.handleReconcileEvent(aggKey, ctx, evt, stream)
			} else {
				// convert the events
