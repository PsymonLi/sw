...
func createEndpointHandler(infraAPI types.InfraAPI, epClient halapi.EndpointClient, intfClient halapi.InterfaceClient, endpoint netproto.Endpoint, vrfID, networkID uint64, collectorMap map[uint64]int) error {
	...
	if createEnic {
		isEPMigrating := false
		if isMigrating(endpoint) && isMigratingIn(endpoint, infraAPI.GetDscName()) {
...
func convertEPAttrs(addresses []string, mgmtIntf string, usegVLAN uint32, enicID, l2SegID uint64) *halapi.EndpointAttributes {
	...
	if enicID == 0 && l2SegID == types.UntaggedCollVLAN && mgmtIntf == types.NaplesOOBInterface {
		ifKeyHandle = convertIfKeyHandles(0, 0x41030001)[0]
	} else if enicID == 0 {
		...
		ifKeyHandle = convertIfKeyHandles(0, 0x41010001)[0]
	}
