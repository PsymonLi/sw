...
int grpc_ipc::GetLinkStatus(uint32_t port, bool& link_status, uint8_t& link_speed, uint8_t& cable_type, uint8_t *sprom_bytes)
{
    PortGetRequest      *req;
    PortGetRequestMsg   req_msg;
    PortGetResponseMsg  rsp_msg;
    ...
    if (status.ok()) {
        if (port_handle_api_status(
            ...
            cable_type = rsp_msg.response(0).status().xcvr_status().cable_type();
            memcpy(sprom_bytes, (char *)(rsp_msg.response(0).status().xcvr_status().xcvr_sprom().data()), 96);
