...
$PathToArtifactsListFile = $PSScriptRoot+"\PenUtilArtifactsList.txt"
...
#
$BuildVersionFile = "PenUtil-" + $VerMaj + "." + $VerMin + "." + $VerSP + ".txt"
...
$Env:PENUTIL_VERSION_MAJOR = $VerMaj

$Env:PENUTIL_VERSION_MINOR = $VerMin

$Env:PENUTIL_VERSION_SP = $VerSP

$Env:PENUTIL_VERSION_BUILD = $VerBuild

$Env:PENUTIL_VERSION_EXT = $VerExt

$Env:PENUTIL_VERSION_STR = $VerStr
...
$BuildLogFile = $BuildLogPath + "PenUtilClean.log"
...
nuget.exe restore $SolutionNameAndPath
# Check for nuget error.
if ($LASTEXITCODE -ne 0) {
    Write-Host -BackgroundColor Red "Nuget restore operation failed.Exit code: "$LASTEXITCODE
    PauseThenExit -ExitCode $LASTEXITCODE
}
...
$BuildLogFile = $BuildLogPath + "PenUtilRebuild.log"
