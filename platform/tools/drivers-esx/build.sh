...
if  [[ $1 = "--install" ]]; then
    ...
    i=0
    for FILENAME in `(find platform/drivers/esxi/ionic_en/build/vib/ -name \ionic-en*.vib)`
    do
        i=$((i + 1))
        if [ $i -gt 1 ]; then
    ...
    if [ $1 -eq 67 ]
    then
        cd platform/drivers/esxi/ionic_en/ && cp Makefile_6.7 Makefile && make
    ...
    then
        cd platform/drivers/esxi/ionic_en/ && cp Makefile_6.5 Makefile && make
        mkdir -p .build/package/devel/linux32/release/nativeddk/NativeDDK/rpm/ionic_en/output/current/vib20/ionic-en && mv .build/package/devel/linux32/release/nativeddk/NativeDDK/rpm/ionic_en/output/current/*.vib .build/package/devel/linux32/release/nativeddk/NativeDDK/rpm/ionic_en/output/current/vib20/ionic-en
        ...
        cd platform/drivers/esxi/ionic_en/ && cp Makefile_7.0 Makefile && cp ionic_7.0.sc.no ionic.sc && make
        rm -rf .build
    fi
